...
namespace MappingTable
{
    class SetFamily
    {
        ...
        internal static bool IsSTB2table = false;
        ...
        internal static bool LoadTable()
        {
            ...
            bool ret = false;
            string familyTableFile = Commons.RexJPath(Commons.ConvRFA_tbl);
            try
            {
                ...
                if (IsSTB2table)
                {
                    Commons.FoundationText2[2] = new string[] { "RC杭", "", "鋼管杭", "既製杭 PHC", "既製杭 ST", "既製杭 SC", "既製杭 PRC", "既製杭 CPRC" };
                }
                else
                {
                    Commons.FoundationText2[2] = new string[] { "場所打ち杭", "既製杭" };
                }
                ...
                do
                {
                    ...
                    if (split.Length > 1)
                    {
                        if (split[0] == "Version")
                        {
                            ...
                            ret &= double.TryParse(Commons.RFAtableVersion, out double v2);
                    ...
                    if (split.Length < 3) //ファミリ名の設定
                    {
                        ...
                        //STB2.0対応
                        if (hed == "RC杭") { CastinPile.FamilyName = dat; IsSTB2table = true; }
        ...
        {
            bool ret = false;
            string TableFile = Commons.RexJPath() + Commons.ConvBase_tbl;
            try
            {
        ...
        {
            const string Ver = Commons.RevitVersion;
