...
namespace CmdDuctDisplacement.UI.Commands.DuctConnection
{
    ...
    /// ↓ボタン押下時のイベント関連クラス
    /// </summary>
    class BottomArrangmentCommand : ICommand
    {
        //メンバ変数
        ...
        //コンストラクタ
        #region Constructor
        public BottomArrangmentCommand(ConnectionSettingViewModel ductPropaty, Logger _log, MEPOperation _mep)
        {
            ductp = ductPropaty;
            CommandSwitch = false;
            mep = _mep;
        ...
        {
            if (ductp.BottomArrangementColor == Brushes.LightSteelBlue.ToString() &&
                CommandSwitch == true)
            {
                return false;
            }
        ...
        public void Execute(object parameter)
        {
            ...
            controlstatus.CallRoute = DuctDisplacementDefine.TextChangeRoute.BottomArrangmentButton;
            ...
            log.Trace("CalculateDiff param in offsetPos:" + offsetpos + "direction:" + DuctDisplacementDefine.DIR_DOWN +
            ...
            mep.CalculateDiff(out hDiff, out clearance, out offset,
                offsetpos, DuctDisplacementDefine.DIR_DOWN, cont.RoundType(ductp), xx, FireProofingValue);
