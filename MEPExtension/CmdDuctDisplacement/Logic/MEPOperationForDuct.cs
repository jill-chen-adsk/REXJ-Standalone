...
{
    public class MEPOperationForDuct : MEPOperation 
    {
        #region メンバ変数
        #endregion
        ...
        public MEPOperationForDuct(Autodesk.Revit.ApplicationServices.Application app, UIDocument uidoc, Logger log) : base(app, uidoc, log){}
        #endregion
        ...
        /// 離隔の場合の移動量算出
        /// ※（メモ）すべて内部の計算は、参照レベルからのオフセット値で計算。
        /// ※（メモ）参照レベルとは、ここでは、1点目選択時のアクティブビューの参照レベル。
        /// </summary>
            ...
            int offsetPos, int direction, int roundUnit, double minClear, double insulate)
        {
            ...
            Parameter middleParam = duct.get_Parameter(BuiltInParameter.RBS_OFFSET_PARAM);
            ...
            switch (direction)
            {
                case DuctDisplacementDefine.DIR_UPPER:
                    ...
                    ductHeight = (double)((decimal)bottomParam.AsDouble() + (decimal)duct.ReferenceLevel.Elevation - (decimal)_mep.GetInsulationThickness(duct) - (decimal)view.GenLevel.Elevation);
                    log.Trace("RBS_DUCT_BOTTOM_ELEVATION:" + bottomParam.AsDouble());
                ...
                case DuctDisplacementDefine.DIR_DOWN:
                    ...
                    ductHeight = (double)((decimal)topParam.AsDouble() + (decimal)_mep.GetInsulationThickness(duct) + (decimal)duct.ReferenceLevel.Elevation - (decimal)view.GenLevel.Elevation);
                    log.Trace("RBS_DUCT_TOP_ELEVATION:" + topParam.AsDouble());
            ...
            /*
            log.Trace("ダクト参照レベル" + _geometry.ConvertFeetToMillimeters(duct.ReferenceLevel.Elevation));
            log.Trace("ダクト上面" + _geometry.ConvertFeetToMillimeters(topParam.AsDouble()));
            log.Trace("ダクト中央" + _geometry.ConvertFeetToMillimeters(middleParam.AsDouble()));
            log.Trace("1点入力時参照レベル" + _geometry.ConvertFeetToMillimeters(view.GenLevel.Elevation));
            */
            switch (offsetPos)
            {
                case DuctDisplacementDefine.OFFSET_POS_TOP:
                    // レベルオフセット値算出
                    offset = roundnum.RoundUnnecessaryNum((double)((decimal)_geometry.ConvertFeetToMillimeters(topParam.AsDouble() + duct.ReferenceLevel.Elevation - view.GenLevel.Elevation + hDiffMin) + (decimal)clearExtra));
                    Round(ref offset, roundUnit, direction);
                    // 移動量算出
                    hDiff = roundnum.RoundUnnecessaryNum((double)((decimal)offset - (decimal)_geometry.ConvertFeetToMillimeters(topParam.AsDouble() + duct.ReferenceLevel.Elevation - view.GenLevel.Elevation)));
                    break;
                case DuctDisplacementDefine.OFFSET_POS_MIDDLE:
                    // レベルオフセット値算出
                    offset = roundnum.RoundUnnecessaryNum((double)((decimal)_geometry.ConvertFeetToMillimeters(middleParam.AsDouble() + duct.ReferenceLevel.Elevation - view.GenLevel.Elevation + hDiffMin) + (decimal)clearExtra));
                    Round(ref offset, roundUnit, direction);
                    // 移動量算出
                    hDiff = roundnum.RoundUnnecessaryNum((double)((decimal)offset - (decimal)_geometry.ConvertFeetToMillimeters(middleParam.AsDouble() + duct.ReferenceLevel.Elevation - view.GenLevel.Elevation)));
                    log.Trace("ductoffset:" + middleParam.AsDouble());
                    break;
                case DuctDisplacementDefine.OFFSET_POS_BOTTOM:
                    // レベルオフセット値算出
                    offset = roundnum.RoundUnnecessaryNum((double)((decimal)_geometry.ConvertFeetToMillimeters(bottomParam.AsDouble() + duct.ReferenceLevel.Elevation - view.GenLevel.Elevation + hDiffMin) + (decimal)clearExtra));
                    Round(ref offset, roundUnit, direction);
                    // 移動量算出
                    hDiff = roundnum.RoundUnnecessaryNum((double)((decimal)offset - (decimal)_geometry.ConvertFeetToMillimeters(bottomParam.AsDouble() + duct.ReferenceLevel.Elevation - view.GenLevel.Elevation)));
                    break;
                default:
        ...
        public override bool ModDuctLevelPartially(int movePtn, int fifPtn, double hDiff, ref Dictionary<int, bool> doneSCurveLoad, out string message)
        {
            ...
            double ductHeight = pt1.Z; // 移動ダクトの元の高さ(グローバル)
            ...
            // １ダクト内の区間移動の場合は、ダクトルートによる分岐意外に間に存在しない。
            // 複数にまたがる場合のみ分岐切断を行う。
            if (NeedBranchCut())
            {
            ...
            if (movePtn == (int)DuctDisplacementDefine.MOVE_PTN.OFFSET)
            {
                ...
                // 移動
                if (!MoveMechanicalSystem(hDiff, ref branchConnectInfoList))
                {
                ...
                {
                    ConnectCurves((Duct)info.InDuct, (Duct)info.OutDuct, info.BranchCutPt, fifPtn, hDiff, ref doneSCurveLoad);
                }
                if(result == false)
                {
        ...
        private bool NeedBranchCut()
        {
            ...
            else
            {
                ...
                foreach (Connector con in cons)
                {
                    ...
                    {
                        {
                            if (acon.Owner.Category.Id.Equals(Category.GetCategory(doc, BuiltInCategory.OST_PlaceHolderDucts).Id))
                            {
        ...
        protected override bool ResetInOutDuct(MEPCurve[] cutDuctPair, ElementId mSystemId, ref MEPCurve inDuct, ref MEPCurve outDuct)
        {
            ...
            foreach (Duct duct in cutDuctPair)
            {
                if (_mep.IncludedDuctInMSystem(duct.Id, mSystemId))
                {
                    inDuct = duct;
                    log.Trace("システム内側：" + duct.Id + "mSystemID:" + mSystemId);
                }
                else
                {
                    outDuct = duct;
                    log.Trace("システム外側：" + duct.Id);
                }
            }
            return true;
        }
        ...
        protected override bool UnifyLevel(ElementId systemId, double ductHeight, double hDiffFeet)
        {
            using (Transaction tran = new Transaction(doc))
            {
                ...
                // 切断区間の機械システムに関する情報
                MechanicalSystem mSystem = doc.GetElement(systemId) as MechanicalSystem;
                ElementSet ductNetwork = mSystem.DuctNetwork;
                // ダクトネットワークを順に並べたリスト
                List<ElementId> orderedDuctNetwork = new List<ElementId>();
                if (whichSideDuct == DuctDisplacementDefine.START_SIDE)
                {// １点目入力が始点側の場合
                    GetOrderdSysMemberList(ref orderedDuctNetwork, new List<ElementId>(), curve1);
                }
                else if (whichSideDuct == DuctDisplacementDefine.END_SIDE)
                {// １点目入力が終点側の場合
                    GetOrderdSysMemberList(ref orderedDuctNetwork, new List<ElementId>(), curve2);
                }
                ...
                // レベル統一後のダクトを作成
                foreach (Element e in ductNetwork)
                {
                    if (!(e is Duct)) continue;

                    Duct duct = e as Duct;
                    XYZ sPt = null;
                    XYZ ePt = null;
                    _geometry.GetLocationCurveStartPoint(ref sPt, duct);
                    _geometry.GetLocationCurveEndPoint(ref ePt, duct);
                    ...
                    {
                        Duct newDuct = Duct.Create(doc, mSystem.GetTypeId(), duct.DuctType.Id, duct.ReferenceLevel.Id, sPt, ePt);
                        // 形状に関するパラメータをコピーする
                        // 幅
                        _mep.CopyParam(BuiltInParameter.RBS_CURVE_WIDTH_PARAM, duct, newDuct);
                        // 高さ
                        _mep.CopyParam(BuiltInParameter.RBS_CURVE_HEIGHT_PARAM, duct, newDuct);
                        // 直径
                        _mep.CopyParam(BuiltInParameter.RBS_CURVE_DIAMETER_PARAM, duct, newDuct);
                        //TODO 耐火被覆は？★

                        ductDict.Add(duct.Id, newDuct.Id);
                    }
                    else
                    {
                        // 垂直ダクトの場合（レベル統一でつぶれる）
                        ...
                        _mep.GetConnectionsWithSpMEPElmRef(duct, ref conList);
                        if (conList.Count == 2)
                        {
                ...
                foreach (Element e in ductNetwork)
                {
                    ...
                    {
                        if (!(con.Owner is Duct)) continue;
                        if (tranDuct != null && !con.Origin.IsAlmostEqualTo(lPt.Point)) continue;
                        // ductType == nullの時と、そうでないけど始点に近い方もダクトだった場合
                        tranDuct = con.Owner as Duct;
                    ...
                    if (tranDuct == null)
                    {
                        int idx = orderedDuctNetwork.FindIndex(n => n == e.Id);
                        int i = 2;
                        Element elm = null;
                        while (idx + i < orderedDuctNetwork.Count || idx - i > -1)
                        {
                            if (idx - i > -1)
                            {
                                elm = doc.GetElement(orderedDuctNetwork[idx - i]);
                                if (elm is Duct)
                                {
                                    tranDuct = elm as Duct;
                                    break;
                                }
                            }
                            if (idx + i < orderedDuctNetwork.Count)
                            {
                                elm = doc.GetElement(orderedDuctNetwork[idx + i]);
                                if (elm is Duct)
                                {
                    ...
                    if (tranDuct != null && conList.Count == 2)
                    {
                        ...
                        {
                            newDuct = Duct.Create(doc, mSystem.GetTypeId(), tranDuct.DuctType.Id, tranDuct.ReferenceLevel.Id, pt0, pt1);
                        }
                        else
                        {
                            newDuct = Duct.Create(doc, mSystem.GetTypeId(), tranDuct.DuctType.Id, tranDuct.ReferenceLevel.Id, pt1, pt0);
                        }
                        // 形状に関するパラメータをコピーする
                        // 幅
                        _mep.CopyParam(BuiltInParameter.RBS_CURVE_WIDTH_PARAM, tranDuct, newDuct);
                        // 高さ
                        _mep.CopyParam(BuiltInParameter.RBS_CURVE_HEIGHT_PARAM, tranDuct, newDuct);
                        // 直径
                        _mep.CopyParam(BuiltInParameter.RBS_CURVE_DIAMETER_PARAM, tranDuct, newDuct);
                        //TODO 耐火被覆は？★
                        ductDict.Add(e.Id, newDuct.Id);
                    }
                }
                // レベル統一前にエルボだった部分の接続
                foreach (Element e in ductNetwork)
                {
                    ...
                    if (conList.Count == 2)
                    {
                        if (!ductDict.ContainsKey(conList[0].Owner.Id))
                        {
                            ...
                            // 垂直ダクトに接続していて消えるときくらい？
                            Duct duct = doc.GetElement(ductDict[conList[1].Owner.Id]) as Duct;

                            XYZ pt0 = (conList[0].Origin).Add(new XYZ(0, 0, -(conList[0].Origin).Z + ductHeight + hDiffFeet));
                            XYZ pt1 = (conList[1].Origin).Add(new XYZ(0, 0, -(conList[1].Origin).Z + ductHeight + hDiffFeet));
                            Duct newDuct = null;
                            XYZ pt = null;
                            _geometry.GetLocationCurveStartPoint(ref pt, duct);
                            if (lPt.Point.IsAlmostEqualTo(pt1))
                            {
                                newDuct = Duct.Create(doc, mSystem.GetTypeId(), duct.DuctType.Id, duct.ReferenceLevel.Id, pt1, pt0);
                            }
                            else
                            {
                                newDuct = Duct.Create(doc, mSystem.GetTypeId(), duct.DuctType.Id, duct.ReferenceLevel.Id, pt0, pt1);
                            }
                            // 形状に関するパラメータをコピーする
                            // 幅
                            _mep.CopyParam(BuiltInParameter.RBS_CURVE_WIDTH_PARAM, duct, newDuct);
                        ...
                        else if (!ductDict.ContainsKey(conList[1].Owner.Id))
                        {
                            ...
                            Duct duct = doc.GetElement(ductDict[conList[0].Owner.Id]) as Duct;
                        ...
                        else
                        {
                            ...
                            // memo 接続先がダクト以外（トランジション）であっても、ダクトに変換しているので、
                            //      下記、MEPCurveでも大丈夫のはず。
                            ...
                            if (_geometry.NearlyEquals(angle, 0) || _geometry.NearlyEquals(angle, Math.PI))
                            {
                                    ...
                                    < eCon2.Origin.DistanceTo(sCon1.Origin))
                                {
                                    ...
                                    {
                                        mDuct = Duct.Create(doc, tCurve1.GetTypeId(), tCurve1.ReferenceLevel.Id, eCon1, sCon2);
                                    ...
                                    > eCon2.Origin.DistanceTo(sCon1.Origin))
                                {
                                    ...
                                    {
                                        mDuct = Duct.Create(doc, tCurve2.GetTypeId(), tCurve1.ReferenceLevel.Id, eCon2, sCon1);
        ...
        public override bool CutBranchCurve(ref List<BranchConnectInfo> branchConnectInfoList, ref List<ElementId> sysMemberIds, Element sysMember, MEPCurve ductE)
        {
            ...
            foreach (Connector con in cons)
            {
                ...
                foreach (Connector refCon in refCons)
                {
                    ...
                    if (!sysMemberIds.Contains(refCon.Owner.Id) && !(refCon.Owner is MechanicalSystem)
                                                                    /* systemId != refCon.Owner.Id */)
                    {
                        ...
                        else
                        {
                            ...
                            // その最初のダクトの中点で切断
                            ElementId cutDuctId = ids.Find(id => doc.GetElement(id) is Duct 
                                                    && !doc.GetElement(id).Category.Id.Equals(Category.GetCategory(doc, BuiltInCategory.OST_PlaceHolderDucts).Id));
                            if (cutDuctId == null)
                            {
                                ElementId ductTerminalId = ids.Find(id => doc.GetElement(id) is FamilyInstance
                                                   && (doc.GetElement(id).Category.Id.Equals(Category.GetCategory(doc, BuiltInCategory.OST_DuctTerminal).Id) 
                                                   || (doc.GetElement(id).Category.Id.Equals(Category.GetCategory(doc, BuiltInCategory.OST_MechanicalEquipment).Id))));
                                // 基本的に切断して移動するけど、切断できない場合ターミナルにつながっていなければ、
                                // 移動不可とはしない
                            ...
                            using (Transaction tran = new Transaction(doc))
                            {
                                ...
                                // 中点取得
                                Duct cutDuct = doc.GetElement(cutDuctId) as Duct;
                                XYZ middlePt = null;
                                _geometry.GetLocationCurvePoint(ref middlePt, cutDuct, 2);
                                // ダクトを中点でカット
                                ElementId newDuctId = MechanicalUtils.BreakCurve(doc, cutDuctId, middlePt);
                                ...
                                if (connectedIds.Contains(curve1.Id))
                                {
                                    ...
                                    log.Trace("ブランチ内側ダクト：" + newDuctId.ToString());
                                    log.Trace("ブランチ外側ダクト：" + cutDuctId.ToString());
                                }
                                else
                                {
                                    info.InDuct = doc.GetElement(cutDuctId) as MEPCurve;
                                    info.OutDuct = doc.GetElement(newDuctId) as MEPCurve;
                                    log.Trace("ブランチ内側ダクト：" + cutDuctId.ToString());
                                    log.Trace("ブランチ外側ダクト：" + newDuctId.ToString());
                                }
        ...
        /// 切断指定部の接続
        /// </summary>
        /// <param name="duct">区間内ダクト</param>
        /// <param name="outDuct">区間外ダクト</param>
        ...
        public override bool ConnectCurves(MEPCurve duct, MEPCurve outDuct, XYZ pt, int fifPtn, double hDiff, ref Dictionary<int, bool> doneSCurveLoad)
        {
            ...
            if (isStartSide)
            {
                ...
                switch (fifPtn)
                {
                    case (int)DuctDisplacementDefine.FITTING_PTN.deg90:
                        result = PartialDuctOperationStartSideElbo90((Duct)outDuct, (Duct)duct, hDiff, fifPtn, ref doneSCurveLoad);
                        break;
                    case (int)DuctDisplacementDefine.FITTING_PTN.deg45:
                        result = PartialDuctOperationStartSideElbo45((Duct)outDuct, (Duct)duct, hDiff, fifPtn, ref doneSCurveLoad);
                        break;
                    case (int)DuctDisplacementDefine.FITTING_PTN.S:
                        result = PartialDuctOperationStartSideSCurve((Duct)outDuct, (Duct)duct, hDiff, fifPtn, ref doneSCurveLoad);
                        break;
                }
            }
            else
            {
                ...
                {
                    case (int)DuctDisplacementDefine.FITTING_PTN.deg90:
                        result = PartialDuctOperationEndSideElbo90((Duct)duct, (Duct)outDuct, hDiff, fifPtn, ref doneSCurveLoad);
                        break;
                    case (int)DuctDisplacementDefine.FITTING_PTN.deg45:
                        result = PartialDuctOperationEndSideElbo45((Duct)duct, (Duct)outDuct, hDiff, fifPtn, ref doneSCurveLoad);
                        break;
                    case (int)DuctDisplacementDefine.FITTING_PTN.S:
                        result = PartialDuctOperationEndSideSCurve((Duct)duct, (Duct)outDuct, hDiff, fifPtn, ref doneSCurveLoad);
                        break;
                }
            }
        ...
        /// ダクト切断
        /// </summary>
        /// <param name="cutDuctPair">切断部(区間内側ダクト＆区間外側ダクト)ペア</param>
        /// <param name="duct">切断するダクト</param>
        /// <param name="pt">切断点</param>
        /// <returns></returns>
        protected override bool BreakCurve(ref MEPCurve[] cutDuctPair, ref MEPCurve duct, ref XYZ pt)
        {
            ...
            if (lCurve.Curve.GetEndPoint(0).IsAlmostEqualTo(pt))
            {
                ...
                //{
                //    // システムの端っこであれば、切断しない
                //    if (con.Owner is MechanicalSystem)
                //    {
                //        log.Trace("システム端のダクト端点での切断指定のため、切断しない");
                //        return true;
                //    }
                ...
                foreach (Connector con in sCon.AllRefs)
                {
                    ...
                    // システム途中のダクト端点であれば、コネクタの接続を解除
                    if (!con.Owner.Id.Equals(sCon.Owner.Id) && !(con.Owner is MechanicalSystem))
                    {
                        sCon.DisconnectFrom(con);
                        log.Trace("ダクト端点での切断指定のため、コネクタを解除:" + sCon.Owner.Id + ":" + con.Owner.Id);
                    }
                }
            ...
            else if(lCurve.Curve.GetEndPoint(1).IsAlmostEqualTo(pt))
            {
                ...
                foreach (Connector con in eCon.AllRefs)
                {
                    log.Trace(con.Owner.Name);
                    // システム途中のダクト端点であれば、コネクタの接続を解除
                    if (!con.Owner.Id.Equals(eCon.Owner.Id) && !(con.Owner is MechanicalSystem))
                    {
                        eCon.DisconnectFrom(con);
                        log.Trace("ダクト端点での切断指定のため、コネクタを解除:" + eCon.Owner.Id + ":" + con.Owner.Id);
                    }
                }
            }
            else
            {
                // ptでダクト切断実施
                // ※ブレーク後のダクトの並び(s/~/newduct/duct/~/e)
                log.Trace("duct:" + duct.Id + "  point:" + pt);
                ElementId newDuctId = MechanicalUtils.BreakCurve(doc, duct.Id, pt);
                Duct newDuct = doc.GetElement(newDuctId) as Duct;
                cutDuctPair = new MEPCurve[2] { duct, newDuct };
        ...
        /// <param name="doneSCurveLoad">S管ロード済フラグ辞書(角型ダクト：1, 円型ダクト：2, 楕円型ダクト：3, 配管：4)</param>
        /// <returns></returns>
        public bool PartialDuctOperationStartSideElbo90(Duct outDuct, Duct duct, double hDiff, int fifPtn, ref Dictionary<int, bool> doneSCurveLoad)
        {
            bool result = true;
            using (Transaction tran = new Transaction(doc))
            {
                try
                {
                    ...
                    _mep.ConnectDuctStartSide(outDuct, duct, true);
                    tran.Commit();
                    ...
                    if (tranStatus == TransactionStatus.RolledBack)
                    {
                        ...
                        result = PartialDuctOperationStartSideElbo90Alt(outDuct, duct, hDiff, fifPtn, ref doneSCurveLoad);
                    }
                }
        ...
        /// <param name="doneSCurveLoad">S管ロード済フラグ辞書(角型ダクト：1, 円型ダクト：2, 楕円型ダクト：3, 配管：4)</param>
        /// <returns></returns>
        public bool PartialDuctOperationEndSideElbo90(Duct duct, Duct outDuct, double hDiff, int fifPtn, ref Dictionary<int, bool> doneSCurveLoad)
        {
            bool result = true;
            using (Transaction tran = new Transaction(doc))
            {
                try
                {
                    ...
                    // 処理エラーが出た場合に何も言わずにロールバック
                    FailureRollback(tran);
                    _mep.ConnectDuctEndSide(duct, outDuct, true);
                    tran.Commit();
                    ...
                    {
                        log.Info("処理エラーでロールバック");
                        result = PartialDuctOperationEndSideElbo90Alt(duct, outDuct, hDiff, fifPtn, ref doneSCurveLoad);
                    }
                }
                catch (Autodesk.Revit.Exceptions.ApplicationException ex)
                {
        ...
        /// <param name="doneSCurveLoad">S管ロード済フラグ辞書(角型ダクト：1, 円型ダクト：2, 楕円型ダクト：3, 配管：4)</param>
        /// <returns></returns>
        public bool PartialDuctOperationStartSideElbo45(Duct outDuct, Duct duct, double hDiff, int fifPtn, ref Dictionary<int, bool> doneSCurveLoad)
        {
            WrpGeometry _geometry = new WrpGeometry(uidoc, log);
            bool result = true;
            ...
            using (Transaction tran = new Transaction(doc))
            {
                try
                {
                    ...
                    //**********************
                    XYZ sPt1 = new XYZ();
                    _geometry.GetLocationCurveStartPoint(ref sPt1, outDuct);
                    XYZ ePt1 = new XYZ();
                    _geometry.GetLocationCurveEndPoint(ref ePt1, outDuct);
                    ...
                    if (scalar <= 0 || scalar >= 1) // 端点もダメなので "=" も排除。
                    {
                        ...
                        // 変わりの配置
                        tran.Dispose();
                        result = PartialDuctOperationStartSideElbo45Alt(outDuct, duct, hDiff, fifPtn, ref doneSCurveLoad);
                        return result;
                    }
                    ...
                    // outDuctを短縮する
                    Connector eConOutDuct = _mep.GetEndSideConnector(outDuct);
                    eConOutDuct.Origin = breakPt1;

                    // newDuct, ductを接続
                    _mep.ConnectDuctStartSide(outDuct, duct, false);
        ...
        /// <param name="doneSCurveLoad">S管ロード済フラグ辞書(角型ダクト：1, 円型ダクト：2, 楕円型ダクト：3, 配管：4)</param>
        /// <returns></returns>
        public bool PartialDuctOperationEndSideElbo45(Duct duct, Duct outDuct, double hDiff, int fifPtn, ref Dictionary<int, bool> doneSCurveLoad)
        {
            WrpGeometry _geometry = new WrpGeometry(uidoc, log);
            bool result = true;
            using (Transaction tran = new Transaction(doc))
            {
                try
                {
                    ...
                    //**********************
                    XYZ sPt2 = new XYZ();
                    _geometry.GetLocationCurveStartPoint(ref sPt2, outDuct);
                    XYZ ePt2 = new XYZ();
                    _geometry.GetLocationCurveEndPoint(ref ePt2, outDuct);
                    // 切断点を求める
                    ...
                    if(scalar <= 0 || scalar >= 1) // 端点もダメなので "=" も排除。
                    {
                        ...
                        // 変わりの配置
                        tran.Dispose();
                        result = PartialDuctOperationEndSideElbo45Alt(duct, outDuct, hDiff, fifPtn, ref doneSCurveLoad);
                        return result;
                    }
                    ...
                    // outDuctを短縮する
                    Connector sConOutDuct = _mep.GetStartSideConnector(outDuct);
                    sConOutDuct.Origin = breakPt2;

                    // duct, 2を接続
                    //_mep.ConnectDuct(duct, outDuct);
                    _mep.ConnectDuctEndSide(duct, outDuct, false);
                    tran.Commit();
        ...
        /// <param name="doneSCurveLoad">S管ロード済フラグ辞書(角型ダクト：1, 円型ダクト：2, 楕円型ダクト：3, 配管：4)</param>
        /// <returns></returns>
        public bool PartialDuctOperationStartSideSCurve(Duct outDuct, Duct duct, double hDiff, int fifPtn, ref Dictionary<int, bool> doneSCurveLoad)
        {
            bool result = true;
            using (Transaction tran = new Transaction(doc))
            {
                try
                {
                    ...
                    FailureRollback(tran);
                    // S管で接続
                    result = ConnectSOffset(duct, outDuct, hDiff, ref doneSCurveLoad);
                    ...
                    if (result)
                    {
                        ...
                        {
                            log.Info("処理エラーでロールバック");
                            result = PartialDuctOperationStartSideSCurveAlt(outDuct, duct, hDiff, fifPtn, ref doneSCurveLoad);
                        }
                    }
                    else
                    {
        ...
        /// <param name="doneSCurveLoad">S管ロード済フラグ辞書(角型ダクト：1, 円型ダクト：2, 楕円型ダクト：3, 配管：4)</param>
        /// <returns></returns>
        public bool PartialDuctOperationEndSideSCurve(Duct duct, Duct outDuct, double hDiff, int fifPtn, ref Dictionary<int, bool> doneSCurveLoad)
        {
            bool result = true;
            using (Transaction tran = new Transaction(doc))
            {
                try
                {
                    ...
                    // S管で接続
                    result = ConnectEOffset(duct, outDuct, hDiff, ref doneSCurveLoad);
                    TransactionStatus tranStatus = tran.GetStatus();
                    ...
                    if (result)
                    {
                        ...
                        {
                            log.Info("処理エラーでロールバック");
                            result = PartialDuctOperationEndSideSCurveAlt(duct, outDuct, hDiff, fifPtn, ref doneSCurveLoad);
                        }
                    }
                    else
                    {
        ...
        /// ダクト用オフセット部材のロード
        /// </summary>
        /// <param name="duct">S管に接続するダクト</param>
        /// <param name="sOffset">S管ファミリ</param>
        /// <param name="doneSCurveLoad">S管ロード済フラグ辞書(角型ダクト：1, 円型ダクト：2, 楕円型ダクト：3, 配管：4)</param>
        /// <returns></returns>
        private bool LoadRecOffset(Duct duct, ref FamilySymbol sOffset, ref Dictionary<int, bool> doneSCurveLoad)
        {
            ...
            // ダクト形状確認
            Connector con = _mep.GetStartSideConnector(duct);
            ...
            switch (con.Shape)
            {
                case ConnectorProfileType.Rectangular:
                    ...
                    fileName = ExResources.ResxString(DuctDisplacementDefine.S_CURVE_REC_FILE);
                    filePath = dataFolder + fileName;
                    familyName = ExResources.ResxString(DuctDisplacementDefine.S_CURVE_REC_FAMILY);
                    typeName = ExResources.ResxString(DuctDisplacementDefine.S_CURVE_REC_TYPE);
                    needLoad = doneSCurveLoad[(int)DuctDisplacementDefine.S_CURVE_PTN.RECT];
                    doneSCurveLoad[(int)DuctDisplacementDefine.S_CURVE_PTN.RECT] = true;
                    break;
                case ConnectorProfileType.Round:
                    // 円型ダクト
                    dataFolder = System.IO.Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().Location) + DuctDisplacementDefine.FAMILY_FOLDER;
                    fileName = ExResources.ResxString(DuctDisplacementDefine.S_CURVE_RND_FILE);
                    filePath = dataFolder + fileName;
                    familyName = ExResources.ResxString(DuctDisplacementDefine.S_CURVE_RND_FAMILY);
                    typeName = ExResources.ResxString(DuctDisplacementDefine.S_CURVE_RND_TYPE);
                    needLoad = doneSCurveLoad[(int)DuctDisplacementDefine.S_CURVE_PTN.ROUND];
                    doneSCurveLoad[(int)DuctDisplacementDefine.S_CURVE_PTN.ROUND] = true;
                    break;
                case ConnectorProfileType.Oval:
                    // 楕円型ダクト
                    dataFolder = System.IO.Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().Location) + DuctDisplacementDefine.FAMILY_FOLDER;
                    fileName = ExResources.ResxString(DuctDisplacementDefine.S_CURVE_OVL_FILE);
                    filePath = dataFolder + fileName;
                    familyName = ExResources.ResxString(DuctDisplacementDefine.S_CURVE_OVL_FAMILY);
                    typeName = ExResources.ResxString(DuctDisplacementDefine.S_CURVE_OVL_TYPE);
                    needLoad = doneSCurveLoad[(int)DuctDisplacementDefine.S_CURVE_PTN.OVAL];
                    doneSCurveLoad[(int)DuctDisplacementDefine.S_CURVE_PTN.OVAL] = true;
                    break;
                case ConnectorProfileType.Invalid:
                    break;
            }

            sOffset = _arrange.GetFamilySymbol(familyName, typeName, BuiltInCategory.OST_DuctFitting);
        ...
        /// <param name="doneSCurveLoad">S管ロード済フラグ辞書(角型ダクト：1, 円型ダクト：2, 楕円型ダクト：3, 配管：4)</param>
        /// <returns></returns>
        private bool ConnectSOffset(Duct duct, Duct outDuct, double hDiff, ref Dictionary<int, bool> doneSCurveLoad)
        {
            // S字の形状を表現できるか判定
            if (!CanDrawSCurve(duct, hDiff)) return false;
            // オフセット部材ロード
            FamilySymbol sOffset = null;

            if (!LoadRecOffset(duct, ref sOffset, ref doneSCurveLoad)) return false;
            // outductの向きに合わせて配置
            // オフセット配置
            XYZ pt = null;
            _geometry.GetLocationCurveStartPoint(ref pt, duct);
            FamilyInstance iOffset = null;
            ArrangeMEPFittingStartSide(sOffset, pt, duct, outDuct, hDiff, ref iOffset);

            // ductとオフセット部材接続
            Connector ductCon = _mep.GetStartSideConnector(duct);
            ...
            double lengthOutDuct = ((LocationCurve)outDuct.Location).Curve.Length;
            log.Trace("短縮距離：" + lengthShorten);
            log.Trace("ダクト距離:" + lengthOutDuct);
            ...
            // 断熱材作成
            _mep.CreateDuctInsulation(duct, iOffset.Id);
            return true;
        }
        ...
        /// <param name="doneSCurveLoad">S管ロード済フラグ辞書(角型ダクト：1, 円型ダクト：2, 楕円型ダクト：3, 配管：4)</param>
        /// <returns></returns>
        private bool ConnectEOffset(Duct duct, Duct outDuct, double hDiff, ref Dictionary<int, bool> doneSCurveLoad)
        {
            // S字の形状を表現できるか判定
            if (!CanDrawSCurve(duct, hDiff)) return false;
            // オフセット部材ロード
            ...
            // オフセット配置
            XYZ pt = null;
            _geometry.GetLocationCurveEndPoint(ref pt, duct);
            FamilyInstance iOffset = null;
            ArrangeMEPFittingEndSide(sOffset, pt, duct, outDuct, hDiff, ref iOffset);
            // ductとオフセット部材接続
            Connector ductCon = _mep.GetEndSideConnector(duct);
            Connector offsetCon = _mep.FindFittingConnector(iOffset, true);
            ductCon.ConnectTo(offsetCon);
            // outductとオフセット部材接続
            Connector sConOutDuct = _mep.GetStartSideConnector(outDuct);
        ...
        /// <returns></returns>
        private bool ArrangeMEPFittingStartSide(FamilySymbol symbol, XYZ pt, Duct duct, Duct outDuct, double hDiff, ref FamilyInstance fInstance)
        {
            // 配置
            fInstance = doc.Create.NewFamilyInstance(pt, symbol, StructuralType.NonStructural);
            // オフセットパラメータ変更
            SetOffsetParm(duct, hDiff, ref fInstance);
            // オフセット方向に合わせてフリップさせる
            if (hDiff > 0)
            {
        ...
        /// <param name="fInstance">配置したインスタンス</param>
        /// <returns></returns>
        private bool ArrangeMEPFittingEndSide(FamilySymbol symbol, XYZ pt, Duct duct, Duct outDuct, double hDiff, ref FamilyInstance fInstance)
        {
            // 配置
            fInstance = doc.Create.NewFamilyInstance(pt, symbol, StructuralType.NonStructural);
            // オフセットパラメータ変更
        ...
        /// S管ファミリへのパラメータセット
        /// </summary>
        /// <param name="duct">接続するダクト</param>
        /// <param name="hDiff">移動距離</param>
        /// <param name="iOffset">(配置した)S管(インスタンス)</param>
        /// <returns></returns>
        private bool SetOffsetParm(Duct duct, double hDiff, ref FamilyInstance iOffset)
        {
            // オフセットサイズ調整
        ...
        /// <param name="hDiff">移動距離</param>
        /// <returns></returns>
        private bool CanDrawSCurve(Duct duct, double hDiff)
        {
        ...
        private bool PartialDuctOperationStartSideElbo90Alt(Duct outDuct, Duct duct, double hDiff, int fifPtn, ref Dictionary<int, bool> doneSCurveLoad)
        {
            bool result = true;
            ...
            switch (fifPtn)
            {
                case (int)DuctDisplacementDefine.FITTING_PTN.deg90:
                    // 90→45→S
                    result = PartialDuctOperationStartSideElbo45(outDuct, duct, hDiff, fifPtn, ref doneSCurveLoad);
                    break;
                case (int)DuctDisplacementDefine.FITTING_PTN.deg45:
                    // 45→90→
                    result = PartialDuctOperationStartSideSCurve(outDuct, duct, hDiff, fifPtn, ref doneSCurveLoad);
                    break;
                case (int)DuctDisplacementDefine.FITTING_PTN.S:
                    // S→45→90
                    //TODO エラー
        ...
        private bool PartialDuctOperationEndSideElbo90Alt(Duct outDuct, Duct duct, double hDiff, int fifPtn, ref Dictionary<int, bool> doneSCurveLoad)
        {
            bool result = true;
            log.Info("変わりの接続を実施します");
            switch (fifPtn)
            {
                case (int)DuctDisplacementDefine.FITTING_PTN.deg90:
                    // 90→45→S
                    result = PartialDuctOperationEndSideElbo45(outDuct, duct, hDiff, fifPtn, ref doneSCurveLoad);
                    break;
                case (int)DuctDisplacementDefine.FITTING_PTN.deg45:
                    // 45→90→S
                    result = PartialDuctOperationEndSideSCurve(outDuct, duct, hDiff, fifPtn, ref doneSCurveLoad);
                    break;
                case (int)DuctDisplacementDefine.FITTING_PTN.S:
                    // S→45→90
                    return false;
        ...
        private bool PartialDuctOperationStartSideElbo45Alt(Duct outDuct, Duct duct, double hDiff, int fifPtn, ref Dictionary<int, bool> doneSCurveLoad)
        {
            bool result = true;
            log.Info("変わりの接続を実施します");
            switch (fifPtn)
            {
                ...
                case (int)DuctDisplacementDefine.FITTING_PTN.deg45:
                    // 45→90→S
                    result = PartialDuctOperationStartSideElbo90(outDuct, duct, hDiff, fifPtn, ref doneSCurveLoad);
                    break;
                case (int)DuctDisplacementDefine.FITTING_PTN.S:
                    // S→45→90
                    result = PartialDuctOperationStartSideElbo90(outDuct, duct, hDiff, fifPtn, ref doneSCurveLoad);
        ...
        private bool PartialDuctOperationEndSideElbo45Alt(Duct outDuct, Duct duct, double hDiff, int fifPtn, ref Dictionary<int, bool> doneSCurveLoad)
        {
            bool result = true;
            log.Info("変わりの接続を実施します");
            switch (fifPtn)
            {
                ...
                case (int)DuctDisplacementDefine.FITTING_PTN.deg45:
                    // 45→90→S
                    result = PartialDuctOperationEndSideElbo90(outDuct, duct, hDiff, fifPtn, ref doneSCurveLoad);
                    break;
                case (int)DuctDisplacementDefine.FITTING_PTN.S:
                    // S→45→90
                    result = PartialDuctOperationEndSideElbo90(outDuct, duct, hDiff, fifPtn, ref doneSCurveLoad);
        ...
        private bool PartialDuctOperationStartSideSCurveAlt(Duct outDuct, Duct duct, double hDiff, int fifPtn, ref Dictionary<int, bool> doneSCurveLoad)
        {
            bool result = true;
            log.Info("変わりの接続を実施します");
            switch (fifPtn)
        ...
        private bool PartialDuctOperationEndSideSCurveAlt(Duct outDuct, Duct duct, double hDiff, int fifPtn, ref Dictionary<int, bool> doneSCurveLoad)
        {
            bool result = true;
            log.Info("変わりの接続を実施します");
            switch (fifPtn)
        ...
        public override bool ShowMEPSystemMember()
        {
            ...
            // DB内のElementたちでMechanicalSystemを取得
            ICollection<Element> mSystems
              = collector.OfClass(typeof(MechanicalSystem))
                .ToElements();
            ...
            foreach (Element e in mSystems)
            {
                ...
                ElementSet nMember = mSystem.DuctNetwork;
                ElementSet elements = mSystem.Elements;

                string ductNetworkMemberStr = "ductNetworkMemberStr:";

                foreach (Element member in nMember)
                {
                    ductNetworkMemberStr = ductNetworkMemberStr + member.Id + ":";
                }
