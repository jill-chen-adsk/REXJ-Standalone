...
namespace ST_BRIDGE_V2
{
    ...
    public partial class ST_BRIDGE
    {
        ...
        private List<StbExtension> stbExtensionsField;
        ...
        public List<StbExtension> StbExtensions
        {
            get
            {
        ...
        /// <returns></returns>
        public static ST_BRIDGE Read(string path, Encoding encoding_)
        {
            ST_BRIDGE result = null;
            string tempFilePath = "";
            try
            {
                EnsureEncodingProvider();
                var xmlSerializer = new XmlSerializer(typeof(ST_BRIDGE));
                var xmlSettings = new System.Xml.XmlReaderSettings { CheckCharacters = false };
                using (var streamReader = new StreamReader(path, encoding_))
                using (var xmlReader = System.Xml.XmlReader.Create(streamReader, xmlSettings))
                {
                    if (xmlSerializer.CanDeserialize(xmlReader))
                    {
                        result = (ST_BRIDGE)xmlSerializer.Deserialize(xmlReader);
                    }
                    else
                    {
                        string encodingName = DetectFileEncoding(path);
                        var lines = File.ReadAllLines(path, Encoding.GetEncoding(encodingName)).ToList();
                        string fixedXml = FixStbXml(lines);
                        tempFilePath = Path.GetTempFileName();
                        File.WriteAllText(tempFilePath, fixedXml, Encoding.GetEncoding(encodingName));
                        result = ST_BRIDGE.Read(tempFilePath, encoding_);
                    }
                }
            }
            catch (Exception e)
            {
                string message = e.Message.Replace("XML ドキュメント (", "STB ファイル (");
                string instruction = e.InnerException?.Message?.Replace("''", "\"\"") ?? "";
                TaskDialog.Show(message, instruction, TaskDialogCommonButtons.Ok, TaskDialogResult.Ok);
                return null;
            }
            finally
            {
        ...
        /// <summary>
        /// Shift_JISなどの追加エンコーディングを.NETで利用可能にする
        /// </summary>
        ...
        private static string DetectFileEncoding(string path)
        {
            using (var reader = new StreamReader(path, Encoding.ASCII))
            {
                ...
                if (firstLine.Contains("encoding=\"shift_jis\"")) return "Shift_JIS";
                if (firstLine.Contains("encoding=\"utf-8\"")) return "UTF-8";
            }
            Console.WriteLine("Encoding 判別不能");
            return "UTF-8";
        }

        /// <summary>
        /// XML上の問題になりそうな箇所を修正する
        /// </summary>
        /// <param name="lines"></param>
        /// <returns></returns>
        private static string FixStbXml(List<string> lines)
        {
            //  namespace未指定の場合、書き加える
            int i = lines.FindIndex(a => System.Text.RegularExpressions.Regex.IsMatch(a, "^<ST_BRIDGE"));
            if (i >= 0)
            {
                var m = System.Text.RegularExpressions.Regex.Match(lines[i], "<ST_BRIDGE .*?>");
                string m2 = m.Value.Replace(">", " xmlns=\"https://www.building-smart.or.jp/dl\">");
                lines[i] = lines[i].Replace(m.Value, m2);
            }
            ...
            // ヘリオスがSTBの仕様変更に対応できていない部分への対応。
            content = content.Replace("\"REVERSE-T\"", "\"REVERSE_T\"");
            
            // <StbExtensions></StbExtensions>の削除
            string pattern = "<StbExtensions>.*?</StbExtensions>";
            content = Regex.Replace(content, pattern, string.Empty, RegexOptions.Singleline);
        ...
        // /// <param name="path"></param>
        // /// <returns></returns>
        // public static ST_BRIDGE Read(string path)
        // {
        //     ST_BRIDGE result = null;
        //     string path2 = "";
        //     try
        //     {
        //         // デシリアライズする    
        //         var xmlSerializer2 = new XmlSerializer(typeof(ST_BRIDGE));
        ...
        //         using (var xmlReader = System.Xml.XmlReader.Create(streamReader, xmlSettings))
        //         {
        //             if (xmlSerializer2.CanDeserialize(xmlReader))
        //             {
        //                 result = (ST_BRIDGE)xmlSerializer2.Deserialize(xmlReader);
        //             }
        //             else
        //             {
        //
        //                 //文字コード判別
        //                 var encoding = FileEncoding( path ) ;
        //                 Encoding.RegisterProvider(CodePagesEncodingProvider.Instance);
        //                 //namespace未指定の場合、書き加えて保存する
        //                 var s = File.ReadAllLines(path, Encoding.GetEncoding(encoding)).ToList();
        //                 int i = s.FindIndex(a => System.Text.RegularExpressions.Regex.IsMatch(a, @"^<ST_BRIDGE"));
        //                 var m = System.Text.RegularExpressions.Regex.Match(s[i], @"<ST_BRIDGE .*?>");
        //                 string m2 = m.Value.Replace(">", " xmlns=\"https://www.building-smart.or.jp/dl\">");
        //                 s[i] = s[i].Replace(m.Value, m2);
        //
        //                 
        //                 var content =string.Join(Environment.NewLine, s);
        //                 //TRUE,FALSEを小文字化
        //                 content = content.Replace("TRUE", "true").Replace("FALSE", "false");
        //                 //ヘリオスがSTBの仕様変更に対応できていない部分への対応。
        //                 content = content.Replace( "\"REVERSE-T\"", "\"REVERSE_T\"" ) ;
        ...
        //                 Encoding.RegisterProvider(CodePagesEncodingProvider.Instance);
        //                 File.WriteAllText(path2, content, Encoding.GetEncoding(encoding));
        //                 
        //                 // path2 = Path.GetTempFileName();
        //                 // File.WriteAllLines(path2, s.ToArray(), Encoding.GetEncoding("Shift_JIS"));
        //
        //                 //書き加えたものを再読み込み（namespace未指定以外の理由で変換できない場合、namespaceが二重になり結果エラーとなる）
        //                 result = ST_BRIDGE.Read(path2);
        //
        //                 File.Delete(path2);
        //
        //                 string FileEncoding( string path_ )
        //                 {
        //                     string firstLine;
        //                     using (var reader = new StreamReader(path_, Encoding.ASCII))
        //                     {
        //                         firstLine = reader.ReadLine();
        //                     }
        //
        //                     if ( firstLine.ToLower().Contains( "encoding=\"shift_jis\"") ) return "Shift_JIS" ;
        //                     if ( firstLine.ToLower().Contains( "encoding=\"utf-8\"") ) return "UTF-8" ;
        //                     Console.WriteLine("Endcoding 判別不能");
        //                     return  "UTF-8";
        //                 }
        //                 
        //
        ...
        //     }
        //     catch( Exception e ) {
        //         var message = e.Message.Replace( "XML ドキュメント (", "STB ファイル (" ) ;
        //         var instruction = e.InnerException.Message.Replace( "''", "\"\"" ) ;
        //         TaskDialog.Show( message, instruction, TaskDialogCommonButtons.Ok, TaskDialogResult.Ok ) ;
        //
        //         return null ;
        //     }
        ...
        public void Write(string path)
        {
            ...
            {
                // シリアライズする
                var xmlSerializer1 = new XmlSerializer(typeof(ST_BRIDGE));
                var ns = new XmlSerializerNamespaces();
                ns.Add(string.Empty, "https://www.building-smart.or.jp/dl"); //xmlns:=～を消す
                Encoding.RegisterProvider(CodePagesEncodingProvider.Instance);
                using (var streamWriter = new StreamWriter(path, false, Encoding.GetEncoding("Shift_JIS")))
                {
                    xmlSerializer1.Serialize(streamWriter, this, ns);
                    streamWriter.Flush();
                }
            }
            catch
        ...
        public void SetApplyConditionsList()
        {
            if (StbCommon.StbApplyConditionsList == null) return;

            if (StbCommon.StbApplyConditionsList.StbColumn_RC_RebarPositionApply != null &&
                StbCommon.StbApplyConditionsList.StbColumn_RC_RebarPositionApply.set_default)
            {
                var apply = StbCommon.StbApplyConditionsList.StbColumn_RC_RebarPositionApply;

                foreach (var col in StbModel.StbSections.StbSecColumn_RC)
                {
                    if (col.StbSecBarArrangementColumn_RC != null)
                    {
                        var bar = col.StbSecBarArrangementColumn_RC;

                        if (bar.depth_cover_start_X <= 0.01) bar.depth_cover_start_X = apply.depth_cover;
                        if (bar.depth_cover_start_Y <= 0.01) bar.depth_cover_start_Y = apply.depth_cover;
                        if (bar.depth_cover_end_X <= 0.01) bar.depth_cover_end_X = apply.depth_cover;
                        if (bar.depth_cover_end_Y <= 0.01) bar.depth_cover_end_Y = apply.depth_cover;
                        if (bar.interval <= 0.01) bar.interval = apply.interval;
                        if (bar.center_start_X <= 0.01) bar.center_start_X = apply.center;
                        if (bar.center_start_Y <= 0.01) bar.center_start_Y = apply.center;
                        if (bar.center_end_X <= 0.01) bar.center_end_X = apply.center;
                        if (bar.center_end_Y <= 0.01) bar.center_end_Y = apply.center;
                        if (bar.center_interval <= 0.01) bar.center_interval = apply.length_to_center;
                    }
                }
            }

            if (StbCommon.StbApplyConditionsList.StbColumn_RC_BarSpacingApply != null &&
                StbCommon.StbApplyConditionsList.StbColumn_RC_BarSpacingApply.set_default)
            {
                var apply = StbCommon.StbApplyConditionsList.StbColumn_RC_BarSpacingApply;

                foreach (var col in StbModel.StbSections.StbSecColumn_RC)
                {
                    if (col.StbSecBarArrangementColumn_RC != null)
                    {
                        foreach (var b in col.StbSecBarArrangementColumn_RC.Items)
                        {
                            if (b is StbSecBarColumn_RC_RectSame b1)
                            {
                                if (b1.D_bar_spacing.Trim() == "") b1.D_bar_spacing = apply.D_bar_spacing;
                                if (b1.pitch_bar_spacing <= 0.01) b1.pitch_bar_spacing = apply.pitch_bar_spacing;
                                if (b1.N_bar_spacing_X == 0) b1.N_bar_spacing_X = apply.N_bar_spacing_X;
                                if (b1.N_bar_spacing_Y == 0) b1.N_bar_spacing_Y = apply.N_bar_spacing_Y;
                            }
                            else if (b is StbSecBarColumn_RC_RectNotSame b2)
                            {
                                if (b2.D_bar_spacing.Trim() == "") b2.D_bar_spacing = apply.D_bar_spacing;
                                if (b2.pitch_bar_spacing <= 0.01) b2.pitch_bar_spacing = apply.pitch_bar_spacing;
                                if (b2.N_bar_spacing_X == 0) b2.N_bar_spacing_X = apply.N_bar_spacing_X;
                                if (b2.N_bar_spacing_Y == 0) b2.N_bar_spacing_Y = apply.N_bar_spacing_Y;
                            }
                            else if (b is StbSecBarColumn_RC_CircleSame b3)
                            {
                                if (b3.D_bar_spacing.Trim() == "") b3.D_bar_spacing = apply.D_bar_spacing;
                                if (b3.pitch_bar_spacing <= 0.01) b3.pitch_bar_spacing = apply.pitch_bar_spacing;
                                if (b3.N_bar_spacing_X == 0) b3.N_bar_spacing_X = apply.N_bar_spacing_X;
                                if (b3.N_bar_spacing_Y == 0) b3.N_bar_spacing_Y = apply.N_bar_spacing_Y;
                            }
                            else if (b is StbSecBarColumn_RC_CircleNotSame b4)
                            {
                                if (b4.D_bar_spacing.Trim() == "") b4.D_bar_spacing = apply.D_bar_spacing;
                                if (b4.pitch_bar_spacing <= 0.01) b4.pitch_bar_spacing = apply.pitch_bar_spacing;
                                if (b4.N_bar_spacing_X == 0) b4.N_bar_spacing_X = apply.N_bar_spacing_X;
                                if (b4.N_bar_spacing_Y == 0) b4.N_bar_spacing_Y = apply.N_bar_spacing_Y;
                            }
                        }
                    }
                }
            ...
            if (StbCommon.StbApplyConditionsList.StbColumn_SRC_RebarPositionApply != null &&
                StbCommon.StbApplyConditionsList.StbColumn_SRC_RebarPositionApply.set_default)
            {
                var apply = StbCommon.StbApplyConditionsList.StbColumn_SRC_RebarPositionApply;

                foreach (var col in StbModel.StbSections.StbSecColumn_SRC)
                {
                    if (col.StbSecBarArrangementColumn_SRC != null)
                    {
                        var bar = col.StbSecBarArrangementColumn_SRC;
            ...
            if (StbCommon.StbApplyConditionsList.StbColumn_SRC_BarSpacingApply != null &&
                StbCommon.StbApplyConditionsList.StbColumn_SRC_BarSpacingApply.set_default)
            {
                var apply = StbCommon.StbApplyConditionsList.StbColumn_SRC_BarSpacingApply;

                foreach (var col in StbModel.StbSections.StbSecColumn_SRC)
                {
                    if (col.StbSecBarArrangementColumn_SRC != null)
                    {
                        foreach (var b in col.StbSecBarArrangementColumn_SRC.Items)
                        {
                            if (b is StbSecBarColumn_SRC_RectSame b1)
                            {
                                if (b1.D_bar_spacing.Trim() == "") b1.D_bar_spacing = apply.D_bar_spacing;
                                if (b1.pitch_bar_spacing <= 0.01) b1.pitch_bar_spacing = apply.pitch_bar_spacing;
                                if (b1.N_bar_spacing_X == 0) b1.N_bar_spacing_X = apply.N_bar_spacing_X;
                            ...
                            else if (b is StbSecBarColumn_SRC_RectNotSame b2)
                            {
                                if (b2.D_bar_spacing.Trim() == "") b2.D_bar_spacing = apply.D_bar_spacing;
                                if (b2.pitch_bar_spacing <= 0.01) b2.pitch_bar_spacing = apply.pitch_bar_spacing;
                                if (b2.N_bar_spacing_X == 0) b2.N_bar_spacing_X = apply.N_bar_spacing_X;
                            ...
                            else if (b is StbSecBarColumn_SRC_CircleSame b3)
                            {
                                if (b3.D_bar_spacing.Trim() == "") b3.D_bar_spacing = apply.D_bar_spacing;
                                if (b3.pitch_bar_spacing <= 0.01) b3.pitch_bar_spacing = apply.pitch_bar_spacing;
                                if (b3.N_bar_spacing_X == 0) b3.N_bar_spacing_X = apply.N_bar_spacing_X;
                            ...
                            else if (b is StbSecBarColumn_SRC_CircleNotSame b4)
                            {
                                if (b4.D_bar_spacing.Trim() == "") b4.D_bar_spacing = apply.D_bar_spacing;
                                if (b4.pitch_bar_spacing <= 0.01) b4.pitch_bar_spacing = apply.pitch_bar_spacing;
                                if (b4.N_bar_spacing_X == 0) b4.N_bar_spacing_X = apply.N_bar_spacing_X;
            ...
            if (StbCommon.StbApplyConditionsList.StbBeam_RC_RebarPositionApply != null &&
                StbCommon.StbApplyConditionsList.StbBeam_RC_RebarPositionApply.set_default)
            {
                var apply = StbCommon.StbApplyConditionsList.StbBeam_RC_RebarPositionApply;

                foreach (var beam in StbModel.StbSections.StbSecBeam_RC)
                {
                    if (beam.StbSecBarArrangementBeam_RC != null)
                    {
                        var bar = beam.StbSecBarArrangementBeam_RC;

                        if (bar.depth_cover_left <= 0.01) bar.depth_cover_left = apply.depth_cover_side;
                        if (bar.depth_cover_right <= 0.01) bar.depth_cover_right = apply.depth_cover_side;
                        if (bar.depth_cover_top <= 0.01) bar.depth_cover_top = apply.depth_cover_top_bottom;
                        if (bar.depth_cover_bottom <= 0.01) bar.depth_cover_bottom = apply.depth_cover_top_bottom;
                        if (bar.interval <= 0.01) bar.interval = apply.interval;
                        if (bar.center_side <= 0.01) bar.center_side = apply.center_side;
                        if (bar.center_top <= 0.01) bar.center_top = apply.center_top_bottom;
                        if (bar.center_bottom <= 0.01) bar.center_bottom = apply.center_top_bottom;
                        if (bar.center_interval <= 0.01) bar.center_interval = apply.length_to_center;
                    }
                }
            }

            if (StbCommon.StbApplyConditionsList.StbBeam_RC_BarWebApply != null &&
                StbCommon.StbApplyConditionsList.StbBeam_RC_BarWebApply.set_default)
            {
                var apply = StbCommon.StbApplyConditionsList.StbBeam_RC_BarWebApply;

                foreach (var beam in StbModel.StbSections.StbSecBeam_RC)
                {
                    if (beam.StbSecBarArrangementBeam_RC != null)
                    {
                        foreach (var b in beam.StbSecBarArrangementBeam_RC.Items)
                        {
                            if (b is StbSecBarBeam_RC_Same b1)
                            {
                                if (b1.D_web.Trim() == "") b1.D_web = apply.D_web;
                                if (b1.N_web == 0) b1.N_web = apply.N_web;
                            }
                            else if (b is StbSecBarBeam_RC_StartEnd b2)
                            {
                                if (b2.D_web.Trim() == "") b2.D_web = apply.D_web;
                                if (b2.N_web == 0) b2.N_web = apply.N_web;
                            }
                            else if (b is StbSecBarBeam_RC_ThreeTypes b3)
                            {
                                if (b3.D_web.Trim() == "") b3.D_web = apply.D_web;
                                if (b3.N_web == 0) b3.N_web = apply.N_web;
                            }
                        }
                    }
                }
            ...
            if (StbCommon.StbApplyConditionsList.StbBeam_RC_BarSpacingApply != null &&
                StbCommon.StbApplyConditionsList.StbBeam_RC_BarSpacingApply.set_default)
            {
                var apply = StbCommon.StbApplyConditionsList.StbBeam_RC_BarSpacingApply;
                ...
                foreach (var beam in StbModel.StbSections.StbSecBeam_RC)
                {
                    if (beam.StbSecBarArrangementBeam_RC != null)
                    {
                        foreach (var b in beam.StbSecBarArrangementBeam_RC.Items)
                        {
                            ...
                            {
                                if (b1.D_bar_spacing.Trim() == "") b1.D_bar_spacing = apply.D_bar_spacing;
                                if (b1.pitch_bar_spacing == 0) b1.pitch_bar_spacing = apply.pitch_bar_spacing;
                                if (b1.N_bar_spacing == 0) b1.N_bar_spacing = apply.N_bar_spacing;
                            }
                            else if (b is StbSecBarBeam_RC_StartEnd b2)
                            {
                                if (b2.D_bar_spacing.Trim() == "") b2.D_bar_spacing = apply.D_bar_spacing;
                                if (b2.pitch_bar_spacing == 0) b2.pitch_bar_spacing = apply.pitch_bar_spacing;
                                if (b2.N_bar_spacing == 0) b2.N_bar_spacing = apply.N_bar_spacing;
                            }
                            else if (b is StbSecBarBeam_RC_ThreeTypes b3)
                            {
                                if (b3.D_bar_spacing.Trim() == "") b3.D_bar_spacing = apply.D_bar_spacing;
                                if (b3.pitch_bar_spacing == 0) b3.pitch_bar_spacing = apply.pitch_bar_spacing;
                                if (b3.N_bar_spacing == 0) b3.N_bar_spacing = apply.N_bar_spacing;
                            }
                        }
                    }
                }
            ...
            if (StbCommon.StbApplyConditionsList.StbBeam_SRC_RebarPositionApply != null &&
                StbCommon.StbApplyConditionsList.StbBeam_SRC_RebarPositionApply.set_default)
            {
                var apply = StbCommon.StbApplyConditionsList.StbBeam_SRC_RebarPositionApply;
            ...
            if (StbCommon.StbApplyConditionsList.StbBeam_SRC_BarWebApply != null &&
                StbCommon.StbApplyConditionsList.StbBeam_SRC_BarWebApply.set_default)
            {
                var apply = StbCommon.StbApplyConditionsList.StbBeam_SRC_BarWebApply;
                ...
                foreach (var beam in StbModel.StbSections.StbSecBeam_RC)
                {
                    if (beam.StbSecBarArrangementBeam_RC != null)
                    {
                        foreach (var b in beam.StbSecBarArrangementBeam_RC.Items)
                        {
                            if (b is StbSecBarBeam_SRC_Same b1)
                            {
                                if (b1.D_web.Trim() == "") b1.D_web = apply.D_web;
                                if (b1.N_web == 0) b1.N_web = apply.N_web;
                            }
                            else if (b is StbSecBarBeam_SRC_StartEnd b2)
                            {
                                if (b2.D_web.Trim() == "") b2.D_web = apply.D_web;
                                if (b2.N_web == 0) b2.N_web = apply.N_web;
                            }
                            else if (b is StbSecBarBeam_SRC_ThreeTypes b3)
                            {
                                if (b3.D_web.Trim() == "") b3.D_web = apply.D_web;
                                if (b3.N_web == 0) b3.N_web = apply.N_web;
                            }
            ...
            if (StbCommon.StbApplyConditionsList.StbBeam_SRC_BarSpacingApply != null &&
                StbCommon.StbApplyConditionsList.StbBeam_SRC_BarSpacingApply.set_default)
            {
                var apply = StbCommon.StbApplyConditionsList.StbBeam_SRC_BarSpacingApply;
            ...
            if (StbCommon.StbApplyConditionsList.StbSlab_RC_BarPositionApply != null &&
                StbCommon.StbApplyConditionsList.StbSlab_RC_BarPositionApply.set_default)
            {
                var apply = StbCommon.StbApplyConditionsList.StbSlab_RC_BarPositionApply;

                foreach (var slab in StbModel.StbSections.StbSecSlab_RC)
                {
                    if (slab.StbSecBarArrangementSlab_RC != null)
                    {
                        var bar = slab.StbSecBarArrangementSlab_RC;

                        if (bar.depth_cover_top <= 0.01) bar.depth_cover_top = apply.depth_cover;
                        if (bar.depth_cover_bottom <= 0.01) bar.depth_cover_bottom = apply.depth_cover;
                    }
                }

                foreach (var slab in StbModel.StbSections.StbSecSlabDeck)
                {
                    if (slab.StbSecBarArrangementSlabDeck != null)
                    {
                        var bar = slab.StbSecBarArrangementSlabDeck;
                ...
                foreach (var slab in StbModel.StbSections.StbSecSlabPrecast)
                {
                    if (slab.StbSecBarArrangementSlabPrecast != null)
                    {
                        var bar = slab.StbSecBarArrangementSlabPrecast;
            ...
            if (StbCommon.StbApplyConditionsList.StbWall_RC_BarPositionApply != null &&
                StbCommon.StbApplyConditionsList.StbWall_RC_BarPositionApply.set_default)
            {
                var apply = StbCommon.StbApplyConditionsList.StbWall_RC_BarPositionApply;

                foreach (var wall in StbModel.StbSections.StbSecWall_RC)
                {
                    if (wall.StbSecBarArrangementWall_RC != null)
                    {
                        var bar = wall.StbSecBarArrangementWall_RC;

                        if (bar.depth_cover_inside <= 0.01) bar.depth_cover_inside = apply.depth_cover;
                        if (bar.depth_cover_outside <= 0.01) bar.depth_cover_outside = apply.depth_cover;
                    }
                }
            }
            ...
            if (StbCommon.StbApplyConditionsList.StbFoundation_RC_BarPositionApply != null &&
                StbCommon.StbApplyConditionsList.StbFoundation_RC_BarPositionApply.set_default)
            {
                var apply = StbCommon.StbApplyConditionsList.StbFoundation_RC_BarPositionApply;

                foreach (var foundation in StbModel.StbSections.StbSecFoundation_RC)
                {
                    if (foundation.StbSecBarArrangementFoundation_RC != null)
                    {
                        var bar = foundation.StbSecBarArrangementFoundation_RC;

                        if (bar.depth_cover_top <= 0.01) bar.depth_cover_top = apply.depth_cover;
                        if (bar.depth_cover_bottom <= 0.01) bar.depth_cover_bottom = apply.depth_cover;
                        if (bar.depth_cover_side <= 0.01) bar.depth_cover_side = apply.depth_cover;
                    }
                }
            }
            ...
            if (StbCommon.StbApplyConditionsList.StbPile_RC_BarPositionApply != null &&
                StbCommon.StbApplyConditionsList.StbPile_RC_BarPositionApply.set_default)
            {
                var apply = StbCommon.StbApplyConditionsList.StbPile_RC_BarPositionApply;

                foreach (var pile in StbModel.StbSections.StbSecPile_RC)
                {
                    if (pile.StbSecBarArrangementPile_RC != null)
                    {
                        var bar = pile.StbSecBarArrangementPile_RC;

                        if (bar.depth_cover <= 0.01) bar.depth_cover = apply.depth_cover;
                        if (bar.depth_cover_top <= 0.01) bar.depth_cover_top = apply.depth_cover;
                    }
                }
            }
            ...
            if (StbCommon.StbApplyConditionsList.StbParapet_RC_BarPositionApply != null &&
                StbCommon.StbApplyConditionsList.StbParapet_RC_BarPositionApply.set_default)
            {
                var apply = StbCommon.StbApplyConditionsList.StbParapet_RC_BarPositionApply;

                foreach (var parapet in StbModel.StbSections.StbSecParapet_RC)
                {
                    if (parapet.StbSecBarArrangementParapet_RC != null)
                    {
                        var bar = parapet.StbSecBarArrangementParapet_RC;
    ...
    public partial class StbCommon
    {
        ...
        private List<StbReinforcementStrength> stbReinforcementStrengthListField;
        ...
        public List<StbReinforcementStrength> StbReinforcementStrengthList
        {
            get
            {
    ...
    public partial class StbColumn_RC_BarSpacingApply
    {
        ...
        private int n_bar_spacing_XField;

        private int n_bar_spacing_YField;
    ...
    public partial class StbBeam_RC_BarSpacingApply
    {
        ...
        private int n_bar_spacingField;
    ...
    public partial class StbModel
    {
        ...
        private List<StbNode> stbNodesField;

        private StbAxes stbAxesField;

        private List<StbStory> stbStoriesField;
        ...
        public List<StbStory> StbStories
        {
            get
            {
    ...
    public partial class StbAxes
    {
        ...
        private List<StbParallelAxes> stbParallelAxesField;

        private List<StbArcAxes> stbArcAxesField;

        private List<StbRadialAxes> stbRadialAxesField;

        private StbDrawingAxes StbDrawingAxesField;

        /// <remarks/>
        [System.Xml.Serialization.XmlElementAttribute("StbParallelAxes")]
        public List<StbParallelAxes> StbParallelAxes
        {
            get
            {
        ...
        public List<StbArcAxes> StbArcAxes
        {
            get
            {
        ...
        public List<StbRadialAxes> StbRadialAxes
        {
            get
            {
    ...
    public partial class StbParallelAxes
    {
        ...
        private List<StbParallelAxis> stbParallelAxisField;
        ...
        public List<StbParallelAxis> StbParallelAxis
        {
            get
            {
    ...
    public partial class StbParallelAxis
    {
        ...
        private List<StbNodeId> stbNodeIdListField;
        ...
        public List<StbNodeId> StbNodeIdList
        {
            get
            {
    ...
    public partial class StbArcAxes
    {
        ...
        private List<StbArcAxis> stbArcAxisField;
        ...
        public List<StbArcAxis> StbArcAxis
        {
            get
            {
    ...
    public partial class StbRadialAxes
    {
        ...
        private List<StbRadialAxis> stbRadialAxisField;
        ...
        public List<StbRadialAxis> StbRadialAxis
        {
            get
            {
    ...
    public partial class StbDrawingAxes
    {
        ...
        private List<StbDrawingLineAxis> stbDrawingLineAxisField;

        private List<StbDrawingArcAxis> stbDrawingArcAxisField;

        /// <remarks/>
        [System.Xml.Serialization.XmlElementAttribute("StbDrawingLineAxis")]
        public List<StbDrawingLineAxis> StbDrawingLineAxis
        {
            get
            {
        ...
        public List<StbDrawingArcAxis> StbDrawingArcAxis
        {
            get
            {
    ...
    public partial class StbStory
    {
        ...
        private int id_dependenceField;
    ...
    public partial class StbMembers
    {
        ...
        private List<StbColumn> stbColumnsField;

        private List<StbPost> stbPostsField;

        private List<StbGirder> stbGirdersField;

        private List<StbBeam> stbBeamsField;

        private List<StbBrace> stbBracesField;

        private List<StbSlab> stbSlabsField;

        private List<StbWall> stbWallsField;

        private List<StbFooting> stbFootingsField;

        private List<StbStripFooting> stbStripFootingsField;

        private List<StbPile> stbPilesField;

        private List<StbFoundationColumn> stbFoundationColumnsField;

        private List<StbParapet> stbParapetsField;

        private List<StbOpen> stbOpensField;

        /// <remarks/>
        [System.Xml.Serialization.XmlArrayItemAttribute("StbColumn", IsNullable = false)]
        public List<StbColumn> StbColumns
        {
            get
            {
        ...
        public List<StbGirder> StbGirders
        {
            get
            {
        ...
        public List<StbBrace> StbBraces
        {
            get
            {
        ...
        public List<StbFooting> StbFootings
        {
            get
            {
        ...
        public List<StbStripFooting> StbStripFootings
        {
            get
            {
        ...
        public List<StbFoundationColumn> StbFoundationColumns
        {
            get
            {
        ...
        public List<StbParapet> StbParapets
        {
            get
            {
    ...
    public partial class StbColumn
    {
        ...
        private int id_node_bottomField;

        private int id_node_topField;
        ...
        private StbColumnCondition condition_bottomField;

        private StbColumnCondition condition_topField;

        private double joint_topField;

        private double joint_bottomField;

        private StbColumnKind_joint kind_joint_topField;

        private StbColumnKind_joint kind_joint_bottomField;

        private int joint_id_topField;

        private int joint_id_bottomField;

        public StbColumn()
        {
            this.rotateField = 0D;
            this.condition_bottomField = StbColumnCondition.FIX;
            this.condition_topField = StbColumnCondition.FIX;
            this.kind_joint_topField = StbColumnKind_joint.WELD;
            this.kind_joint_bottomField = StbColumnKind_joint.WELD;
        }
        public StbColumn(StbColumn c)
        {
            this.idField = c.id;
            this.guidField = c.guid;
            this.nameField = c.name;
            this.id_node_bottomField = c.id_node_bottom;
            this.id_node_topField = c.id_node_top;
            this.rotateField = c.rotate;
            this.id_sectionField = c.id_section;
            this.kind_structureField = c.kind_structure;
            this.offset_bottom_XField = c.offset_bottom_X;
            this.offset_bottom_YField = c.offset_bottom_Y;
            this.offset_bottom_ZField = c.offset_bottom_Z;
            this.offset_top_XField = c.offset_top_X;
            this.offset_top_YField = c.offset_top_Y;
            this.offset_top_ZField = c.offset_top_Z;
            this.thickness_add_start_XField = c.thickness_add_start_X;
            this.thickness_add_end_XField = c.thickness_add_end_X;
            this.thickness_add_start_YField = c.thickness_add_start_Y;
            this.thickness_add_end_YField = c.thickness_add_end_Y;
            this.condition_bottomField = c.condition_bottom;
            this.condition_topField = c.condition_top;
            this.joint_topField = c.joint_top;
            this.joint_bottomField = c.joint_bottom;
            this.kind_joint_topField = c.kind_joint_top;
            this.kind_joint_bottomField = c.kind_joint_bottom;
            this.joint_id_topField = c.joint_id_top;
            this.joint_id_bottomField = c.joint_id_bottom;
        }
        ...
        /// <remarks/>
        [System.Xml.Serialization.XmlAttributeAttribute()]
        [System.ComponentModel.DefaultValueAttribute(StbColumnCondition.FIX)]
        public StbColumnCondition condition_bottom
        {
            get
            {
        ...
        [System.Xml.Serialization.XmlAttributeAttribute()]
        [System.ComponentModel.DefaultValueAttribute(StbColumnCondition.FIX)]
        public StbColumnCondition condition_top
        {
            get
            {
        ...
        /// <remarks/>
        [System.Xml.Serialization.XmlAttributeAttribute()]
        [System.ComponentModel.DefaultValueAttribute(StbColumnKind_joint.WELD)]
        public StbColumnKind_joint kind_joint_top
        {
            get
            {
        ...
        [System.Xml.Serialization.XmlAttributeAttribute()]
        [System.ComponentModel.DefaultValueAttribute(StbColumnKind_joint.WELD)]
        public StbColumnKind_joint kind_joint_bottom
        {
            get
            {
    ...
    [System.SerializableAttribute()]
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    public enum StbColumnCondition
    {
    ...
    [System.SerializableAttribute()]
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    public enum StbColumnKind_joint
    {
    ...
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    [System.Xml.Serialization.XmlRootAttribute(Namespace = "https://www.building-smart.or.jp/dl", IsNullable = false)]
    public partial class StbPost : StbColumn
    {
        public StbPost() : base()
        {
        }
        public StbPost(StbColumn c) : base(c)
        {
        }
    }
    ...
    public partial class StbGirder
    {
        ...
        private int id_node_startField;

        private int id_node_endField;

        private double rotateField;

        private int id_sectionField;

        private StbGirderSection_io section_io_startField;

        private StbGirderSection_io section_io_endField;
        ...
        private StbGirderCondition condition_startField;

        private StbGirderCondition condition_endField;
        ...
        private StbGirderKind_haunch kind_haunch_startField;

        private StbGirderKind_haunch kind_haunch_endField;

        private StbGirderType_haunch_H type_haunch_HField;

        private StbGirderType_haunch_V type_haunch_VField;

        private StbGirderKind_joint kind_joint_startField;

        private StbGirderKind_joint kind_joint_endField;

        private int joint_id_startField;

        private int joint_id_endField;

        public StbGirder()
        {
            this.rotateField = 0D;
            this.section_io_startField = StbGirderSection_io.NONE;
            this.section_io_endField = StbGirderSection_io.NONE;
            this.condition_startField = StbGirderCondition.FIX;
            this.condition_endField = StbGirderCondition.FIX;
            this.kind_haunch_startField = StbGirderKind_haunch.SLOPE;
            this.kind_haunch_endField = StbGirderKind_haunch.SLOPE;
            this.type_haunch_HField = StbGirderType_haunch_H.BOTH;
            this.type_haunch_VField = StbGirderType_haunch_V.BOTH;
            this.kind_joint_startField = StbGirderKind_joint.WELD;
            this.kind_joint_endField = StbGirderKind_joint.WELD;
        }
        public StbGirder(StbGirder g)
        {
            this.idField = g.id;
            this.guidField = g.guid;
            this.nameField = g.name;
            this.id_node_startField = g.id_node_start;
            this.id_node_endField = g.id_node_end;
            this.rotateField = g.rotate;
            this.id_sectionField = g.id_section;
            this.section_io_startField = g.section_io_start;
            this.section_io_endField = g.section_io_end;
            this.kind_structureField = g.kind_structure;
            this.isFoundationField = g.isFoundation;
            this.offset_start_XField = g.offset_start_X;
            this.offset_start_YField = g.offset_start_Y;
            this.offset_start_ZField = g.offset_start_Z;
            this.offset_end_XField = g.offset_end_X;
            this.offset_end_YField = g.offset_end_Y;
            this.offset_end_ZField = g.offset_end_Z;
            this.thickness_add_topField = g.thickness_add_top;
            this.thickness_add_bottomField = g.thickness_add_bottom;
            this.thickness_add_rightField = g.thickness_add_right;
            this.thickness_add_leftField = g.thickness_add_left;
            this.condition_startField = g.condition_start;
            this.condition_endField = g.condition_end;
            this.haunch_startField = g.haunch_start;
            this.haunch_endField = g.haunch_end;
            this.joint_startField = g.joint_start;
            this.joint_endField = g.joint_end;
            this.kind_haunch_startField = g.kind_haunch_start;
            this.kind_haunch_endField = g.kind_haunch_end;
            this.type_haunch_HField = g.type_haunch_H;
            this.type_haunch_VField = g.type_haunch_V;
            this.kind_joint_startField = g.kind_joint_start;
            this.kind_joint_endField = g.kind_joint_end;
            this.joint_id_startField = g.joint_id_start;
            this.joint_id_endField = g.joint_id_end;
        }
        ...
        /// <remarks/>
        [System.Xml.Serialization.XmlAttributeAttribute()]
        [System.ComponentModel.DefaultValueAttribute(StbGirderSection_io.NONE)]
        public StbGirderSection_io section_io_start
        {
            get
            {
        ...
        [System.Xml.Serialization.XmlAttributeAttribute()]
        [System.ComponentModel.DefaultValueAttribute(StbGirderSection_io.NONE)]
        public StbGirderSection_io section_io_end
        {
            get
            {
        ...
        /// <remarks/>
        [System.Xml.Serialization.XmlAttributeAttribute()]
        [System.ComponentModel.DefaultValueAttribute(StbGirderCondition.FIX)]
        public StbGirderCondition condition_start
        {
            get
            {
        ...
        [System.Xml.Serialization.XmlAttributeAttribute()]
        [System.ComponentModel.DefaultValueAttribute(StbGirderCondition.FIX)]
        public StbGirderCondition condition_end
        {
            get
            {
        ...
        /// <remarks/>
        [System.Xml.Serialization.XmlAttributeAttribute()]
        [System.ComponentModel.DefaultValueAttribute(StbGirderKind_haunch.SLOPE)]
        public StbGirderKind_haunch kind_haunch_start
        {
            get
            {
        ...
        [System.Xml.Serialization.XmlAttributeAttribute()]
        [System.ComponentModel.DefaultValueAttribute(StbGirderKind_haunch.SLOPE)]
        public StbGirderKind_haunch kind_haunch_end
        {
            get
            {
        ...
        /// <remarks/>
        [System.Xml.Serialization.XmlAttributeAttribute()]
        [System.ComponentModel.DefaultValueAttribute(StbGirderKind_joint.WELD)]
        public StbGirderKind_joint kind_joint_start
        {
            get
            {
        ...
        [System.Xml.Serialization.XmlAttributeAttribute()]
        [System.ComponentModel.DefaultValueAttribute(StbGirderKind_joint.WELD)]
        public StbGirderKind_joint kind_joint_end
        {
            get
            {
    ...
    [System.SerializableAttribute()]
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    public enum StbGirderSection_io
    {
        NONE,
    ...
    [System.SerializableAttribute()]
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    public enum StbGirderCondition
    {
    ...
    [System.SerializableAttribute()]
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    public enum StbGirderKind_haunch
    {
    ...
    [System.SerializableAttribute()]
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    public enum StbGirderKind_joint
    {
    ...
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    [System.Xml.Serialization.XmlRootAttribute(Namespace = "https://www.building-smart.or.jp/dl", IsNullable = false)]
    public partial class StbBeam : StbGirder
    {
        public StbBeam() : base()
        {
        }
        public StbBeam(StbGirder g) : base(g)
        {
        }
    }
    ...
    public partial class StbBrace
    {
        ...
        private StbBraceCondition condition_startField;

        private StbBraceCondition condition_endField;
        ...
        private StbBraceKind_joint kind_joint_startField;

        private StbBraceKind_joint kind_joint_endField;
        ...
        {
            this.rotateField = 0D;
            this.condition_startField = StbBraceCondition.FIX;
            this.condition_endField = StbBraceCondition.FIX;
            this.feature_braceField = StbBraceFeature_brace.TENSION;
            this.kind_joint_startField = StbBraceKind_joint.WELD;
            this.kind_joint_endField = StbBraceKind_joint.WELD;
        }
        ...
        /// <remarks/>
        [System.Xml.Serialization.XmlAttributeAttribute()]
        [System.ComponentModel.DefaultValueAttribute(StbBraceCondition.FIX)]
        public StbBraceCondition condition_start
        {
            get
            {
                return this.condition_startField;
        ...
        [System.Xml.Serialization.XmlAttributeAttribute()]
        [System.ComponentModel.DefaultValueAttribute(StbBraceCondition.FIX)]
        public StbBraceCondition condition_end
        {
            get
            {
                return this.condition_endField;
        ...
        /// <remarks/>
        [System.Xml.Serialization.XmlAttributeAttribute()]
        [System.ComponentModel.DefaultValueAttribute(StbBraceKind_joint.WELD)]
        public StbBraceKind_joint kind_joint_start
        {
            get
            {
                return this.kind_joint_startField;
        ...
        [System.Xml.Serialization.XmlAttributeAttribute()]
        [System.ComponentModel.DefaultValueAttribute(StbBraceKind_joint.WELD)]
        public StbBraceKind_joint kind_joint_end
        {
            get
            {
                return this.kind_joint_endField;
    ...
    [System.SerializableAttribute()]
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    public enum StbBraceKind_joint
    {
    ...
    public partial class StbSlab
    {
        ...
        private List<StbSlabOffset> stbSlabOffsetListField;

        private List<StbOpenId> stbOpenIdListField;
        ...
        public List<int> StbNodeIdOrderList
        {
            get
            {
                List<int> id = new List<int>();
                foreach (var s in this.stbNodeIdOrderField.Split(' '))
                {
        ...
        public List<StbSlabOffset> StbSlabOffsetList
        {
            get
            {
        ...
        public List<StbOpenId> StbOpenIdList
        {
            get
            {
        ...
        /// <remarks/>
        [System.Xml.Serialization.XmlAttributeAttribute()]
        [System.ComponentModel.DefaultValueAttribute(StbSlabDirection_load.NONE)]
        public StbSlabDirection_load direction_load
        {
            get
            {
                if (this.direction_loadField == StbSlabDirection_load.NONE)
                {
                    if (this.kind_slab == StbSlabKind_slab.NORMAL) return StbSlabDirection_load.Item2WAY;
                    if (this.kind_slab == StbSlabKind_slab.CANTI) return StbSlabDirection_load.Item1WAY;
                }
                return this.direction_loadField;
            }
            set
            {
                if (value == StbSlabDirection_load.NONE)
                {
                    if (this.kind_slab == StbSlabKind_slab.NORMAL) this.direction_loadField = StbSlabDirection_load.Item2WAY;
                    else if (this.kind_slab == StbSlabKind_slab.CANTI) this.direction_loadField = StbSlabDirection_load.Item1WAY;
                    else this.direction_loadField = value;
                }
                else
                {
        ...
        public double angle_load
        {
            ...
            set
            {
                ...
                if (Math.Abs(this.angle_loadField) > 0.001)
                {
                    angle_loadSpecified = true;
                }
                else if (this.direction_load == StbSlabDirection_load.Item1WAY)
                {
                    angle_loadSpecified = true;
                }
            }
        ...
        /// <remarks/>
        [System.Xml.Serialization.XmlAttributeAttribute()]
        [System.ComponentModel.DefaultValueAttribute(StbSlabType_haunch.BOTTOM)]
    ...
    {
        /// <summary>スラブ種類がNORMAL => 2Way, スラブ種類がCANTI => 1way</summary>
        NONE,
    ...
    public partial class StbWall
    {
        ...
        private List<StbWallOffset> stbWallOffsetListField;
        ...
        public List<StbWallOffset> StbWallOffsetList
        {
            get
            {
        ...
        /// <remarks/>
        [System.Xml.Serialization.XmlAttributeAttribute()]
        [System.ComponentModel.DefaultValueAttribute(StbWallType_outside.NONE)]
    ...
    public partial class StbFoundationColumn
    {
        ...
        private int id_section_FDField;
        ...
        private int id_section_WRField;
    ...
    public partial class StbParapet
    {
        ...
        /// <remarks/>
        [System.Xml.Serialization.XmlAttributeAttribute()]
        [System.ComponentModel.DefaultValueAttribute(StbParapetDirection.NONE)]
    ...
    public partial class StbSections
    {
        ...
        private List<StbSecColumn_RC> stbSecColumn_RCField;

        private List<StbSecColumn_S> stbSecColumn_SField;

        private List<StbSecColumn_SRC> stbSecColumn_SRCField;

        private List<StbSecColumn_CFT> stbSecColumn_CFTField;

        private List<StbSecBeam_RC> stbSecBeam_RCField;

        private List<StbSecBeam_S> stbSecBeam_SField;

        private List<StbSecBeam_SRC> stbSecBeam_SRCField;

        private List<StbSecBrace_S> stbSecBrace_SField;

        private List<StbSecSlab_RC> stbSecSlab_RCField;

        private List<StbSecSlabDeck> stbSecSlabDeckField;

        private List<StbSecSlabPrecast> stbSecSlabPrecastField;

        private List<StbSecWall_RC> stbSecWall_RCField;

        private List<StbSecFoundation_RC> stbSecFoundation_RCField;

        private List<StbSecPile_RC> stbSecPile_RCField;

        private List<StbSecPile_S> stbSecPile_SField;

        private List<StbSecPileProduct> stbSecPileProductField;

        private List<StbSecOpen_RC> stbSecOpen_RCField;

        private List<StbSecParapet_RC> stbSecParapet_RCField;

        private StbSecSteel stbSecSteelField;

        /// <remarks/>
        [System.Xml.Serialization.XmlElementAttribute("StbSecColumn_RC")]
        public List<StbSecColumn_RC> StbSecColumn_RC
        {
            get
            {
        ...
        public List<StbSecColumn_S> StbSecColumn_S
        {
            get
            {
        ...
        public List<StbSecColumn_SRC> StbSecColumn_SRC
        {
            get
            {
        ...
        public List<StbSecColumn_CFT> StbSecColumn_CFT
        {
            get
            {
        ...
        public List<StbSecBeam_RC> StbSecBeam_RC
        {
            get
            {
        ...
        public List<StbSecBeam_S> StbSecBeam_S
        {
            get
            {
        ...
        public List<StbSecBeam_SRC> StbSecBeam_SRC
        {
            get
            {
        ...
        public List<StbSecBrace_S> StbSecBrace_S
        {
            get
            {
        ...
        public List<StbSecSlab_RC> StbSecSlab_RC
        {
            get
            {
        ...
        public List<StbSecSlabDeck> StbSecSlabDeck
        {
            get
            {
        ...
        public List<StbSecSlabPrecast> StbSecSlabPrecast
        {
            get
            {
        ...
        public List<StbSecWall_RC> StbSecWall_RC
        {
            get
            {
        ...
        public List<StbSecFoundation_RC> StbSecFoundation_RC
        {
            get
            {
        ...
        public List<StbSecPile_RC> StbSecPile_RC
        {
            get
            {
        ...
        public List<StbSecPile_S> StbSecPile_S
        {
            get
            {
        ...
        public List<StbSecPileProduct> StbSecPileProduct
        {
            get
            {
        ...
        public List<StbSecOpen_RC> StbSecOpen_RC
        {
            get
            {
        ...
        public List<StbSecParapet_RC> StbSecParapet_RC
        {
            get
            {
    ...
    public partial class StbSecColumn_RC
    {
        ...
        private StbSecColumn_Kind_column kind_columnField;
        ...
        /// <remarks/>
        [System.Xml.Serialization.XmlAttributeAttribute()]
        [System.ComponentModel.DefaultValueAttribute(StbSecColumn_Kind_column.COLUMN)]
        public StbSecColumn_Kind_column kind_column
        {
            get
            {
    ...
    public partial class StbSecBarColumnXReinforced
    {
        ...
        private int n_main_XField;

        private int n_main_YField;

        private int n_main_totalField;
    ...
    public partial class StbSecBarColumn_RC_CircleNotSame
    {
        ...
        private StbSecBarColumn_RC_NotSamePos posField;
        ...
        private int n_mainField;

        private int n_axialField;

        private int n_bandField;
        ...
        public StbSecBarColumn_RC_CircleNotSame()
        {
        }

        public StbSecBarColumn_RC_CircleNotSame(StbSecBarColumn_RC_CircleSame same) : this()
        {
            if (same == null) return;

            this.d_mainField = same.D_main;
            this.d_axialField = same.D_axial;
            this.d_bandField = same.D_band;
            this.d_bar_spacingField = same.D_bar_spacing;
            this.strength_mainField = same.strength_main;
            this.strength_axialField = same.strength_axial;
            this.strength_bandField = same.strength_band;
            this.strength_bar_spacingField = same.strength_bar_spacing;
            this.n_mainField = same.N_main;
            this.n_axialField = same.N_axial;
            this.n_bandField = same.N_band;
            this.pitch_bandField = same.pitch_band;
            this.pitch_bar_spacingField = same.pitch_bar_spacing;
            this.n_bar_spacing_XField = same.N_bar_spacing_X;
            this.n_bar_spacing_YField = same.N_bar_spacing_Y;
        }


        /// <remarks/>
        [System.Xml.Serialization.XmlAttributeAttribute()]
        public StbSecBarColumn_RC_NotSamePos pos
        {
            get
            {
    ...
    public partial class StbSecBarColumn_RC_RectNotSame
    {
        ...
        private int n_main_X_1stField;

        private int n_main_X_2ndField;

        private int n_main_Y_1stField;

        private int n_main_Y_2ndField;

        private int n_2nd_main_X_1stField;

        private int n_2nd_main_X_2ndField;

        private int n_2nd_main_Y_1stField;

        private int n_2nd_main_Y_2ndField;
        ...
        private int n_band_direction_XField;

        private int n_band_direction_YField;
        ...
        public StbSecBarColumn_RC_RectNotSame()
        {
        }

        public StbSecBarColumn_RC_RectNotSame(StbSecBarColumn_RC_RectSame same) : this()
        {
            if (same == null) return;

            this.d_mainField = same.D_main;
            this.d_2nd_mainField = same.D_2nd_main;
            this.d_axialField = same.D_axial;
            this.d_bandField = same.D_band;
            this.d_bar_spacingField = same.D_bar_spacing;
            this.strength_mainField = same.strength_main;
            this.strength_2nd_mainField = same.strength_2nd_main;
            this.strength_axialField = same.strength_axial;
            this.strength_bandField = same.strength_band;
            this.strength_bar_spacingField = same.strength_bar_spacing;
            this.n_main_X_1stField = same.N_main_X_1st;
            this.n_main_X_2ndField = same.N_main_X_2nd;
            this.n_main_Y_1stField = same.N_main_Y_1st;
            this.n_main_Y_2ndField = same.N_main_Y_2nd;
            this.n_2nd_main_X_1stField = same.N_2nd_main_X_1st;
            this.n_2nd_main_X_2ndField = same.N_2nd_main_X_2nd;
            this.n_2nd_main_Y_1stField = same.N_2nd_main_Y_1st;
            this.n_2nd_main_Y_2ndField = same.N_2nd_main_Y_2nd;
            this.n_main_totalField = same.N_main_total;
            this.n_axialField = same.N_axial;
            this.pitch_bandField = same.pitch_band;
            this.n_band_direction_XField = same.N_band_direction_X;
            this.n_band_direction_YField = same.N_band_direction_Y;
            this.pitch_bar_spacingField = same.pitch_bar_spacing;
            this.n_bar_spacing_XField = same.N_bar_spacing_X;
            this.n_bar_spacing_YField = same.N_bar_spacing_Y;
        }
    ...
    [System.SerializableAttribute()]
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    public enum StbSecBarColumn_RC_NotSamePos
    {
    ...
    [System.SerializableAttribute()]
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    public enum StbSecColumn_Kind_column
    {
    ...
    public partial class StbSecBaseConventional_S_AnchorBolt
    {
        ...
        private string arrangement_boltField;
        ...
        private int n_XField;

        private int n_YField;
        ...
        /// <remarks/>
        [System.Xml.Serialization.XmlAttributeAttribute()]
        public string arrangement_bolt
        {
            get
            {
    ...
    [System.SerializableAttribute()]
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    public enum StbSecBaseProduct_SDirection_type : int
    {
    ...
    public partial class StbSecColumn_SRC
    {
        ...
        {
            this.kind_columnField = StbSecColumn_Kind_column.COLUMN;
        }
    ...
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    [System.Xml.Serialization.XmlRootAttribute(Namespace = "https://www.building-smart.or.jp/dl", IsNullable = false)]
    public partial class StbSecBarColumn_SRC_CircleNotSame : StbSecBarColumn_RC_CircleNotSame
    {
        public StbSecBarColumn_SRC_CircleNotSame() : base()
        {
        }

        public StbSecBarColumn_SRC_CircleNotSame(StbSecBarColumn_SRC_CircleSame same) : base(same)
        {
        }
    }
    ...
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    [System.Xml.Serialization.XmlRootAttribute(Namespace = "https://www.building-smart.or.jp/dl", IsNullable = false)]
    public partial class StbSecBarColumn_SRC_CircleSame : StbSecBarColumn_RC_CircleSame
    {
    }
    ...
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    [System.Xml.Serialization.XmlRootAttribute(Namespace = "https://www.building-smart.or.jp/dl", IsNullable = false)]
    public partial class StbSecBarColumn_SRC_RectNotSame : StbSecBarColumn_RC_RectNotSame
    {
        public StbSecBarColumn_SRC_RectNotSame() : base()
        {
        }

        public StbSecBarColumn_SRC_RectNotSame(StbSecBarColumn_SRC_RectSame same) : base(same)
        {
        }
    }
    ...
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    [System.Xml.Serialization.XmlRootAttribute(Namespace = "https://www.building-smart.or.jp/dl", IsNullable = false)]
    public partial class StbSecBarColumn_SRC_RectSame : StbSecBarColumn_RC_RectSame
    {
    }
    ...
    public partial class StbSecSteelFigureColumn_SRC
    {
        ...
        /// <summary>
        /// 鉄骨形状タイプ [1:H, 2:Box, 3:Pipe, 4:Cross, 5:T]
        /// </summary>
        [System.Xml.Serialization.XmlIgnoreAttribute()]
        internal int SteelType
        {
            get
            {
                if (Items != null)
                {
                    var src1 = Items.OfType<StbSecSteelColumn_SRC_Same>().FirstOrDefault();
                    var src2 = Items.OfType<StbSecSteelColumn_SRC_NotSame>().FirstOrDefault();
                    var src3 = Items.OfType<StbSecSteelColumn_SRC_ThreeTypes>().FirstOrDefault();

                    if (src1 != null)
                    {
                        if (src1.Item is StbSecColumn_SRC_SameShapeH)
                        {
                            return 1;
                        }
                        else if (src1.Item is StbSecColumn_SRC_SameShapeBox)
                        {
                            return 2;
                        }
                        else if (src1.Item is StbSecColumn_SRC_SameShapePipe)
                        {
                            return 3;
                        }
                        else if (src1.Item is StbSecColumn_SRC_SameShapeCross)
                        {
                            return 4;
                        }
                        else if (src1.Item is StbSecColumn_SRC_SameShapeT)
                        {
                            return 5;
                        }
                    }
                    ...
                    {
                        if (src2.Item is StbSecColumn_SRC_NotSameShapeH)
                        {
                            return 1;
                        }
                        else if (src2.Item is StbSecColumn_SRC_NotSameShapeBox)
                        {
                            return 2;
                        }
                        else if (src2.Item is StbSecColumn_SRC_NotSameShapePipe)
                        {
                            return 3;
                        }
                        else if (src2.Item is StbSecColumn_SRC_NotSameShapeCross)
                        {
                            return 4;
                        }
                        else if (src2.Item is StbSecColumn_SRC_NotSameShapeT)
                        {
                            return 5;
                        }
                    }
                    ...
                    {
                        if (src3.Item is StbSecColumn_SRC_ThreeTypesShapeH)
                        {
                            return 1;
                        }
                        else if (src3.Item is StbSecColumn_SRC_ThreeTypesShapeBox)
                        {
                            return 2;
                        }
                        else if (src3.Item is StbSecColumn_SRC_ThreeTypesShapePipe)
                        {
                            return 3;
                        }
                        else if (src3.Item is StbSecColumn_SRC_ThreeTypesShapeCross)
                        {
                            return 4;
                        }
                        else if (src3.Item is StbSecColumn_SRC_ThreeTypesShapeT)
                        {
                            return 5;
                        }
                    }
    ...
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    [System.Xml.Serialization.XmlRootAttribute(Namespace = "https://www.building-smart.or.jp/dl", IsNullable = false)]
    public partial class StbSecColumn_SRC_NotSameShapeBox : StbSecColumn_SRC_SameShapeBox
    {
    }
    ...
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    [System.Xml.Serialization.XmlRootAttribute(Namespace = "https://www.building-smart.or.jp/dl", IsNullable = false)]
    public partial class StbSecColumn_SRC_NotSameShapeCross : StbSecColumn_SRC_SameShapeCross
    {
    }
    ...
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    [System.Xml.Serialization.XmlRootAttribute(Namespace = "https://www.building-smart.or.jp/dl", IsNullable = false)]
    public partial class StbSecColumn_SRC_NotSameShapeH : StbSecColumn_SRC_SameShapeH
    {
    }
    ...
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    [System.Xml.Serialization.XmlRootAttribute(Namespace = "https://www.building-smart.or.jp/dl", IsNullable = false)]
    public partial class StbSecColumn_SRC_NotSameShapePipe : StbSecColumn_SRC_SameShapePipe
    {
    }
    ...
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    [System.Xml.Serialization.XmlRootAttribute(Namespace = "https://www.building-smart.or.jp/dl", IsNullable = false)]
    public partial class StbSecColumn_SRC_NotSameShapeT : StbSecColumn_SRC_SameShapeT
    {
    }
    ...
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    [System.Xml.Serialization.XmlRootAttribute(Namespace = "https://www.building-smart.or.jp/dl", IsNullable = false)]
    public partial class StbSecColumn_SRC_ThreeTypesShapeBox : StbSecColumn_SRC_SameShapeBox
    {
    }
    ...
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    [System.Xml.Serialization.XmlRootAttribute(Namespace = "https://www.building-smart.or.jp/dl", IsNullable = false)]
    public partial class StbSecColumn_SRC_ThreeTypesShapeCross : StbSecColumn_SRC_SameShapeCross
    {
    }
    ...
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    [System.Xml.Serialization.XmlRootAttribute(Namespace = "https://www.building-smart.or.jp/dl", IsNullable = false)]
    public partial class StbSecColumn_SRC_ThreeTypesShapeH : StbSecColumn_SRC_SameShapeH
    {
    }
    ...
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    [System.Xml.Serialization.XmlRootAttribute(Namespace = "https://www.building-smart.or.jp/dl", IsNullable = false)]
    public partial class StbSecColumn_SRC_ThreeTypesShapePipe : StbSecColumn_SRC_SameShapePipe
    {
    }
    ...
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    [System.Xml.Serialization.XmlRootAttribute(Namespace = "https://www.building-smart.or.jp/dl", IsNullable = false)]
    public partial class StbSecColumn_SRC_ThreeTypesShapeT : StbSecColumn_SRC_SameShapeT
    {
    }
    ...
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    [System.Xml.Serialization.XmlRootAttribute(Namespace = "https://www.building-smart.or.jp/dl", IsNullable = false)]
    public partial class StbSecBaseProduct_SRC : StbSecBaseProduct_S
    {
        public StbSecBaseProduct_SRC() : base()
        {
        }
    }
    ...
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    [System.Xml.Serialization.XmlRootAttribute(Namespace = "https://www.building-smart.or.jp/dl", IsNullable = false)]
    public partial class StbSecBaseProduct_CFT : StbSecBaseProduct_S
    {
        public StbSecBaseProduct_CFT() : base()
        {
        }
    }
    ...
    public partial class StbSecBeam_RC
    {
        ...
        private StbSecBeam_Kind_beam kind_beamField;
        ...
        {
            this.kind_beamField = StbSecBeam_Kind_beam.GIRDER;
        ...
        /// <remarks/>
        [System.Xml.Serialization.XmlAttributeAttribute()]
        [System.ComponentModel.DefaultValueAttribute(StbSecBeam_Kind_beam.GIRDER)]
        public StbSecBeam_Kind_beam kind_beam
        {
            get
            {
    ...
    public partial class StbSecFigureBeam_RC
    {
        ...
        /// <summary>
        /// 形状タイプ [1:ストレート, 2:テーパー, 3:ハンチ]
        /// </summary>
        [System.Xml.Serialization.XmlIgnoreAttribute()]
        internal int FigureType
        {
            get
            {
                if (Items != null)
                {
                    if (Items.First() is StbSecBeam_RC_Straight)
                    {
                        return 1;
                    }
                    else if (Items.First() is StbSecBeam_RC_Taper)
                    {
                        return 2;
                    }
                    else if (Items.First() is StbSecBeam_RC_Haunch)
                    {
                        return 3;
                    }
                }
    ...
    public partial class StbSecBarArrangementBeam_RC
    {
        ...
        /// <summary>
        /// 配筋タイプ [1:梁全断面同一配筋, 2:梁断面始端中央終端別配筋, 3:梁断面始端終端別配筋]
        /// </summary>
        [System.Xml.Serialization.XmlIgnoreAttribute()]
        internal int Bar_ArrangementType
        {
            get
            {
                if (Items != null)
                {
                    if (Items.First() is StbSecBarBeam_RC_Same)
                    {
                        return 1;
                    }
                    else if (Items.First() is StbSecBarBeam_RC_ThreeTypes)
                    {
                        return 2;
                    }
                    else if (Items.First() is StbSecBarBeam_RC_StartEnd)
                    {
                        return 3;
                    }
                }
    ...
    public partial class StbSecBarBeam_RC_Same
    {
        ...
        private int n_main_top_1stField;

        private int n_main_top_2ndField;

        private int n_main_top_3rdField;

        private int n_main_bottom_1stField;

        private int n_main_bottom_2ndField;

        private int n_main_bottom_3rdField;

        private int n_2nd_main_top_1stField;

        private int n_2nd_main_top_2ndField;

        private int n_2nd_main_top_3rdField;

        private int n_2nd_main_bottom_1stField;

        private int n_2nd_main_bottom_2ndField;

        private int n_2nd_main_bottom_3rdField;

        private int n_stirrupField;
        ...
        [System.Xml.Serialization.XmlAttributeAttribute()]
        [System.ComponentModel.DefaultValueAttribute(0)]
        public int N_2nd_main_bottom_1st
        {
            get
            {
        ...
        [System.Xml.Serialization.XmlAttributeAttribute()]
        [System.ComponentModel.DefaultValueAttribute(0)]
        public int N_2nd_main_bottom_2nd
        {
            get
            {
        ...
        [System.Xml.Serialization.XmlAttributeAttribute()]
        [System.ComponentModel.DefaultValueAttribute(0)]
        public int N_2nd_main_bottom_3rd
        {
            get
            {
    ...
    public partial class StbSecBarBeam_RC_StartEnd
    {
        ...
        public StbSecBarBeam_RC_StartEnd()
        {
        }

        public StbSecBarBeam_RC_StartEnd(StbSecBarBeam_RC_Same same) : this()
        {
            if (same == null) return;

            this.d_mainField = same.D_main;
            this.d_2nd_mainField = same.D_2nd_main;
            this.d_stirrupField = same.D_stirrup;
            this.d_webField = same.D_web;
            this.d_bar_spacingField = same.D_bar_spacing;
            this.strength_mainField = same.strength_main;
            this.strength_2nd_mainField = same.strength_2nd_main;
            this.strength_stirrupField = same.strength_stirrup;
            this.strength_webField = same.strength_web;
            this.strength_bar_spacingField = same.strength_bar_spacing;
            this.n_main_top_1stField = same.N_main_top_1st;
            this.n_main_top_2ndField = same.N_main_top_2nd;
            this.n_main_top_3rdField = same.N_main_top_3rd;
            this.n_main_bottom_1stField = same.N_main_bottom_1st;
            this.n_main_bottom_2ndField = same.N_main_bottom_2nd;
            this.n_main_bottom_3rdField = same.N_main_bottom_3rd;
            this.n_2nd_main_top_1stField = same.N_2nd_main_top_1st;
            this.n_2nd_main_top_2ndField = same.N_2nd_main_top_2nd;
            this.n_2nd_main_top_3rdField = same.N_2nd_main_top_3rd;
            this.n_2nd_main_bottom_1stField = same.N_2nd_main_bottom_1st;
            this.n_2nd_main_bottom_2ndField = same.N_2nd_main_bottom_2nd;
            this.n_2nd_main_bottom_3rdField = same.N_2nd_main_bottom_3rd;
            this.n_stirrupField = same.N_stirrup;
            this.pitch_stirrupField = same.pitch_stirrup;
            this.n_webField = same.N_web;
            this.n_bar_spacingField = same.N_bar_spacing;
            this.pitch_bar_spacingField = same.pitch_bar_spacing;
        }
    ...
    public partial class StbSecBarBeam_RC_ThreeTypes
    {
        ...
        public StbSecBarBeam_RC_ThreeTypes()
        {
        }

        public StbSecBarBeam_RC_ThreeTypes(StbSecBarBeam_RC_Same same) : this()
        {
            if (same == null) return;
        ...
        public StbSecBarBeam_RC_ThreeTypes(StbSecBarBeam_RC_StartEnd startend) : this()
        {
            if (startend == null) return;

            this.d_mainField = startend.D_main;
            this.d_2nd_mainField = startend.D_2nd_main;
            this.d_stirrupField = startend.D_stirrup;
            this.d_webField = startend.D_web;
            this.d_bar_spacingField = startend.D_bar_spacing;
            this.strength_mainField = startend.strength_main;
            this.strength_2nd_mainField = startend.strength_2nd_main;
            this.strength_stirrupField = startend.strength_stirrup;
            this.strength_webField = startend.strength_web;
            this.strength_bar_spacingField = startend.strength_bar_spacing;
            this.n_main_top_1stField = startend.N_main_top_1st;
            this.n_main_top_2ndField = startend.N_main_top_2nd;
            this.n_main_top_3rdField = startend.N_main_top_3rd;
            this.n_main_bottom_1stField = startend.N_main_bottom_1st;
            this.n_main_bottom_2ndField = startend.N_main_bottom_2nd;
            this.n_main_bottom_3rdField = startend.N_main_bottom_3rd;
            this.n_2nd_main_top_1stField = startend.N_2nd_main_top_1st;
            this.n_2nd_main_top_2ndField = startend.N_2nd_main_top_2nd;
            this.n_2nd_main_top_3rdField = startend.N_2nd_main_top_3rd;
            this.n_2nd_main_bottom_1stField = startend.N_2nd_main_bottom_1st;
            this.n_2nd_main_bottom_2ndField = startend.N_2nd_main_bottom_2nd;
            this.n_2nd_main_bottom_3rdField = startend.N_2nd_main_bottom_3rd;
            this.n_stirrupField = startend.N_stirrup;
            this.pitch_stirrupField = startend.pitch_stirrup;
            this.n_webField = startend.N_web;
            this.n_bar_spacingField = startend.N_bar_spacing;
            this.pitch_bar_spacingField = startend.pitch_bar_spacing;
        }
    ...
    public partial class StbSecBarBeamXReinforced
    {
        ...
        private int n_main_topField;

        private int n_main_bottomField;

        /// <remarks/>
        [System.Xml.Serialization.XmlAttributeAttribute()]
        public int N_main_top
        {
            get
            {
    ...
    [System.SerializableAttribute()]
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    public enum StbSecBeam_Kind_beam
    {
    ...
    public partial class StbSecSteelFigureBeam_S
    {
        ...
        /// <summary>
        /// 形状タイプ [1:ストレート, 2:テーパー, 3:継手あり, 4:ハンチ, 5:５種類]
        /// </summary>
        [System.Xml.Serialization.XmlIgnoreAttribute()]
        internal int FigureType
        {
    ...
    public partial class StbSecFigureBeam_SRC
    {
        ...
        internal int FigureType
        {
            get
            {
                if (Items != null)
                {
                    if (Items.First() is StbSecBeam_SRC_Straight)
                    {
                        return 1;
                    }
                    else if (Items.First() is StbSecBeam_SRC_Taper)
                    {
                        return 2;
                    }
                    else if (Items.First() is StbSecBeam_SRC_Haunch)
                    {
                        return 3;
                    }
                }
    ...
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    [System.Xml.Serialization.XmlRootAttribute(Namespace = "https://www.building-smart.or.jp/dl", IsNullable = false)]
    public partial class StbSecBeam_SRC_Haunch : StbSecBeam_RC_Haunch
    {
    }
    ...
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    [System.Xml.Serialization.XmlRootAttribute(Namespace = "https://www.building-smart.or.jp/dl", IsNullable = false)]
    public partial class StbSecBeam_SRC_Straight : StbSecBeam_RC_Straight
    {
    }
    ...
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    [System.Xml.Serialization.XmlRootAttribute(Namespace = "https://www.building-smart.or.jp/dl", IsNullable = false)]
    public partial class StbSecBeam_SRC_Taper : StbSecBeam_RC_Taper
    {
    }
    ...
    public partial class StbSecBarArrangementBeam_SRC
    {
        ...
        internal int Bar_ArrangementType
        {
            get
            {
                if (Items != null)
                {
                    if (Items.First() is StbSecBarBeam_SRC_Same)
                    {
                        return 1;
                    }
                    else if (Items.First() is StbSecBarBeam_SRC_ThreeTypes)
                    {
                        return 2;
                    }
                    else if (Items.First() is StbSecBarBeam_SRC_StartEnd)
                    {
                        return 3;
                    }
                }
    ...
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    [System.Xml.Serialization.XmlRootAttribute(Namespace = "https://www.building-smart.or.jp/dl", IsNullable = false)]
    public partial class StbSecBarBeam_SRC_Same : StbSecBarBeam_RC_Same
    {
    }
    ...
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    [System.Xml.Serialization.XmlRootAttribute(Namespace = "https://www.building-smart.or.jp/dl", IsNullable = false)]
    public partial class StbSecBarBeam_SRC_StartEnd : StbSecBarBeam_RC_StartEnd
    {
        public StbSecBarBeam_SRC_StartEnd() : base()
        {
        }

        public StbSecBarBeam_SRC_StartEnd(StbSecBarBeam_SRC_Same same) : base(same)
        {
        }
    }
    ...
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    [System.Xml.Serialization.XmlRootAttribute(Namespace = "https://www.building-smart.or.jp/dl", IsNullable = false)]
    public partial class StbSecBarBeam_SRC_ThreeTypes : StbSecBarBeam_RC_ThreeTypes
    {
        public StbSecBarBeam_SRC_ThreeTypes() : base()
        {
        }

        public StbSecBarBeam_SRC_ThreeTypes(StbSecBarBeam_SRC_Same same) : base(same)
        {
        }

        public StbSecBarBeam_SRC_ThreeTypes(StbSecBarBeam_SRC_StartEnd startend) : base(startend)
        {
        }
    }
    ...
    public partial class StbSecSteelFigureBeam_SRC
    {
        ...
        internal int FigureType
        {
            get
            {
                if (Items != null)
                {
                    if (Items.First() is StbSecSteelBeam_SRC_Straight)
                    {
                        return 1;
                    }
                    else if (Items.First() is StbSecSteelBeam_SRC_Taper)
                    {
                        return 2;
                    }
                    else if (Items.First() is StbSecSteelBeam_SRC_Joint)
                    {
                        return 3;
                    }
                    else if (Items.First() is StbSecSteelBeam_SRC_Haunch)
                    {
                        return 4;
                    }
                    else if (Items.First() is StbSecSteelBeam_SRC_FiveTypes)
                    {
                        return 5;
                    }
                }
    ...
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    [System.Xml.Serialization.XmlRootAttribute(Namespace = "https://www.building-smart.or.jp/dl", IsNullable = false)]
    public partial class StbSecSteelBeam_SRC_FiveTypes : StbSecSteelBeam_S_FiveTypes
    {
    }
    ...
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    [System.Xml.Serialization.XmlRootAttribute(Namespace = "https://www.building-smart.or.jp/dl", IsNullable = false)]
    public partial class StbSecSteelBeam_SRC_Haunch : StbSecSteelBeam_S_Haunch
    {
    }
    ...
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    [System.Xml.Serialization.XmlRootAttribute(Namespace = "https://www.building-smart.or.jp/dl", IsNullable = false)]
    public partial class StbSecSteelBeam_SRC_Joint : StbSecSteelBeam_S_Joint
    {
    }
    ...
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    [System.Xml.Serialization.XmlRootAttribute(Namespace = "https://www.building-smart.or.jp/dl", IsNullable = false)]
    public partial class StbSecSteelBeam_SRC_Straight : StbSecSteelBeam_S_Straight
    {
    }
    ...
    [System.Xml.Serialization.XmlTypeAttribute(AnonymousType = true, Namespace = "https://www.building-smart.or.jp/dl")]
    [System.Xml.Serialization.XmlRootAttribute(Namespace = "https://www.building-smart.or.jp/dl", IsNullable = false)]
    public partial class StbSecSteelBeam_SRC_Taper : StbSecSteelBeam_S_Taper
    {
    }
    ...
    public partial class StbSecBarArrangementSlab_RC
    {
        ...
        [System.Xml.Serialization.XmlElementAttribute("StbSecBarSlab_RC_Open", typeof(StbSecBarSlab_RC_Open))]
        public List<object> Items
        {
            get
            {
    ...
    public partial class StbSecBarArrangementWall_RC
    {
        ...
        [System.Xml.Serialization.XmlElementAttribute("StbSecBarWall_RC_Edge", typeof(StbSecBarWall_RC_Edge))]
        [System.Xml.Serialization.XmlElementAttribute("StbSecBarWall_RC_Open", typeof(StbSecBarWall_RC_Open))]
        public List<object> Items
        {
            get
            {
    ...
    public partial class StbSecFigureFoundation_RC
    {
        ...
        /// <summary>
        /// 形状タイプ [1:矩形, 2:矩形テーパー, 3:三角, 4:正三角形, 5:八角形, 6:連続]
        /// </summary>
        [System.Xml.Serialization.XmlIgnoreAttribute()]
        internal int FigureType
        {
            get
            {
                if (Item != null)
                {
                    if (Item is StbSecFoundation_RC_Rect)
                    {
                        return 1;
                    }
                    else if (Item is StbSecFoundation_RC_TaperedRect)
                    {
                        return 2;
                    }
                    else if (Item is StbSecFoundation_RC_Triangle)
                    {
                        return 3;
                    }
                    else if (Item is StbSecFoundation_RC_EquiTriangle)
                    {
                        return 4;
                    }
                    else if (Item is StbSecFoundation_RC_Octagon)
                    {
                        return 5;
                    }
                    else if (Item is StbSecFoundation_RC_Continuous)
                    {
                        return 6;
                    }
                }
    ...
    public partial class StbSecFigurePile_RC
    {
        ...
        /// <summary>
        /// 形状タイプ [1:ストレート, 2:脚部拡大, 3:頂部拡大, 4:両端拡大]
        /// </summary>
        [System.Xml.Serialization.XmlIgnoreAttribute()]
        internal int FigureType
        {
            get
            {
                if (Item != null)
                {
                    if (Item is StbSecPile_RC_Straight)
                    {
                        return 1;
                    }
                    else if (Item is StbSecPile_RC_ExtendedFoot)
                    {
                        return 2;
                    }
                    else if (Item is StbSecPile_RC_ExtendedTop)
                    {
                        return 3;
                    }
                    else if (Item is StbSecPile_RC_ExtendedTopFoot)
                    {
                        return 4;
                    }
                }
    ...
    public partial class StbSecBarPile_RC_Same
    {
        ...
        private int n_main_circumference_1stField;

        private int n_main_circumference_2ndField;

        private int n_main_coreField;
        ...
        /// <remarks/>
        [System.Xml.Serialization.XmlAttributeAttribute()]
        public int N_main_circumference_1st
        {
            get
            {
        ...
        [System.Xml.Serialization.XmlAttributeAttribute()]
        [System.ComponentModel.DefaultValueAttribute(0)]
        public int N_main_circumference_2nd
        {
            get
            {
    ...
    public partial class StbSecFigurePile_S
    {
        ...
        private List<StbSecPile_S_Straight> stbSecPile_S_StraightField;

        private List<StbSecPile_S_Rotational> stbSecPile_S_RotationalField;

        private List<StbSecPile_S_Taper> stbSecPile_S_TaperField;

        /// <remarks/>
        [System.Xml.Serialization.XmlElementAttribute("StbSecPile_S_Straight")]
        public List<StbSecPile_S_Straight> StbSecPile_S_Straight
        {
            get
            {
        ...
        public List<StbSecPile_S_Rotational> StbSecPile_S_Rotational
        {
            get
            {
        ...
        public List<StbSecPile_S_Taper> StbSecPile_S_Taper
        {
            get
            {
    ...
    public partial class StbSecFigurePileProduct
    {
        ...
        private List<StbSecPileProduct_PHC> stbSecPileProduct_PHCField;

        private List<StbSecPileProduct_ST> stbSecPileProduct_STField;

        private List<StbSecPileProduct_SC> stbSecPileProduct_SCField;

        private List<StbSecPileProduct_PRC> stbSecPileProduct_PRCField;

        private List<StbSecPileProduct_CPRC> stbSecPileProduct_CPRCField;

        private List<StbSecPileProductNodular_PHC> stbSecPileProductNodular_PHCField;

        private List<StbSecPileProductNodular_PRC> stbSecPileProductNodular_PRCField;

        private List<StbSecPileProductNodular_CPRC> stbSecPileProductNodular_CPRCField;

        /// <remarks/>
        [System.Xml.Serialization.XmlElementAttribute("StbSecPileProduct_PHC")]
        public List<StbSecPileProduct_PHC> StbSecPileProduct_PHC
        {
            get
            {
        ...
        public List<StbSecPileProduct_ST> StbSecPileProduct_ST
        {
            get
            {
        ...
        public List<StbSecPileProduct_SC> StbSecPileProduct_SC
        {
            get
            {
        ...
        public List<StbSecPileProduct_PRC> StbSecPileProduct_PRC
        {
            get
            {
        ...
        public List<StbSecPileProduct_CPRC> StbSecPileProduct_CPRC
        {
            get
            {
        ...
        public List<StbSecPileProductNodular_PHC> StbSecPileProductNodular_PHC
        {
            get
            {
        ...
        public List<StbSecPileProductNodular_PRC> StbSecPileProductNodular_PRC
        {
            get
            {
        ...
        public List<StbSecPileProductNodular_CPRC> StbSecPileProductNodular_CPRC
        {
            get
            {
    ...
    public partial class StbSecBarArrangementParapet_RC
    {
        ...
        [System.Xml.Serialization.XmlElementAttribute("StbSecBarParapet_RC_Tip", typeof(StbSecBarParapet_RC_Tip))]
        [System.Xml.Serialization.XmlElementAttribute("StbSecBarParapet_RC_Edge", typeof(StbSecBarParapet_RC_Edge))]
        public List<object> Items
        {
            get
            {
    ...
    public partial class StbSecSteel
    {
        ...
        private List<StbSecRollH> stbSecRollHField;

        private List<StbSecBuildH> stbSecBuildHField;

        private List<StbSecRollBox> stbSecRollBoxField;

        private List<StbSecBuildBox> stbSecBuildBoxField;

        private List<StbSecPipe> stbSecPipeField;

        private List<StbSecRollT> stbSecRollTField;

        private List<StbSecRollC> stbSecRollCField;

        private List<StbSecRollL> stbSecRollLField;

        private List<StbSecLipC> stbSecLipCField;

        private List<StbSecFlatBar> stbSecFlatBarField;

        private List<StbSecRoundBar> stbSecRoundBarField;

        /// <remarks/>
        [System.Xml.Serialization.XmlElementAttribute("StbSecRoll-H")]
        public List<StbSecRollH> StbSecRollH
        {
            get
            {
        ...
        public List<StbSecBuildH> StbSecBuildH
        {
            get
            {
        ...
        public List<StbSecRollBox> StbSecRollBox
        {
            get
            {
        ...
        public List<StbSecBuildBox> StbSecBuildBox
        {
            get
            {
        ...
        public List<StbSecPipe> StbSecPipe
        {
            get
            {
        ...
        public List<StbSecRollT> StbSecRollT
        {
            get
            {
        ...
        public List<StbSecRollC> StbSecRollC
        {
            get
            {
        ...
        public List<StbSecRollL> StbSecRollL
        {
            get
            {
        ...
        public List<StbSecLipC> StbSecLipC
        {
            get
            {
        ...
        public List<StbSecFlatBar> StbSecFlatBar
        {
            get
            {
        ...
        public List<StbSecRoundBar> StbSecRoundBar
        {
            get
            {
    ...
    public partial class StbJoints
    {
        ...
        private List<StbJointBeamShapeH> stbJointBeamShapeHField;

        private List<StbJointColumnShapeH> stbJointColumnShapeHField;

        private List<StbJointColumnShapeT> stbJointColumnShapeTField;

        private List<StbJointColumnShapeCross> stbJointColumnShapeCrossField;

        /// <remarks/>
        [System.Xml.Serialization.XmlElementAttribute("StbJointBeamShapeH")]
        public List<StbJointBeamShapeH> StbJointBeamShapeH
        {
            get
            {
        ...
        public List<StbJointColumnShapeH> StbJointColumnShapeH
        {
            get
            {
        ...
        public List<StbJointColumnShapeT> StbJointColumnShapeT
        {
            get
            {
        ...
        public List<StbJointColumnShapeCross> StbJointColumnShapeCross
        {
            get
            {
    ...
    public partial class StbExtension
    {
        ...
        private List<StbExtObject> stbExtObjectField;

        private List<StbExtElement> stbExtElementField;
        ...
        public List<StbExtObject> StbExtObject
        {
            get
            {
        ...
        public List<StbExtElement> StbExtElement
        {
            get
            {
    ...
    public partial class StbExtObject
    {
        ...
        private List<StbExtProperty> stbExtPropertyField;

        private string object_nameField;

        private int id_objectField;

        /// <remarks/>
        [System.Xml.Serialization.XmlElementAttribute("StbExtProperty")]
        public List<StbExtProperty> StbExtProperty
        {
            get
            {
    ...
    public partial class StbExtElement
    {
        ...
        private List<StbExtPropertyDef> stbExtPropertyDefField;
        ...
        public List<StbExtPropertyDef> StbExtPropertyDef
        {
            get
            {
    ...
    public partial class StbReinforcementStrengthList
    {
        ...
        private List<StbReinforcementStrength> stbReinforcementStrengthField;

        /// <remarks/>
        [System.Xml.Serialization.XmlElementAttribute("StbReinforcementStrength")]
        public List<StbReinforcementStrength> StbReinforcementStrength
        {
            get
            {
    ...
    public partial class StbNodes
    {
        ...
        private List<StbNode> stbNodeField;

        /// <remarks/>
        [System.Xml.Serialization.XmlElementAttribute("StbNode")]
        public List<StbNode> StbNode
        {
            get
            {
    ...
    public partial class StbNodeIdList
    {
        ...
        private List<StbNodeId> stbNodeIdField;

        /// <remarks/>
        [System.Xml.Serialization.XmlElementAttribute("StbNodeId")]
        public List<StbNodeId> StbNodeId
        {
            get
            {
    ...
    public partial class StbStories
    {
        ...
        private List<StbStory> stbStoryField;

        /// <remarks/>
        [System.Xml.Serialization.XmlElementAttribute("StbStory")]
        public List<StbStory> StbStory
        {
            get
            {
    ...
    public partial class StbColumns
    {
        ...
        private List<StbColumn> stbColumnField;

        /// <remarks/>
        [System.Xml.Serialization.XmlElementAttribute("StbColumn")]
        public List<StbColumn> StbColumn
        {
            get
            {
    ...
    public partial class StbPosts
    {
        ...
        private List<StbPost> stbPostField;

        /// <remarks/>
        [System.Xml.Serialization.XmlElementAttribute("StbPost")]
        public List<StbPost> StbPost
        {
            get
            {
    ...
    public partial class StbGirders
    {
        ...
        private List<StbGirder> stbGirderField;

        /// <remarks/>
        [System.Xml.Serialization.XmlElementAttribute("StbGirder")]
        public List<StbGirder> StbGirder
        {
            get
            {
    ...
    public partial class StbBeams
    {
        ...
        private List<StbBeam> stbBeamField;

        /// <remarks/>
        [System.Xml.Serialization.XmlElementAttribute("StbBeam")]
        public List<StbBeam> StbBeam
        {
            get
            {
    ...
    public partial class StbBraces
    {
        ...
        private List<StbBrace> stbBraceField;

        /// <remarks/>
        [System.Xml.Serialization.XmlElementAttribute("StbBrace")]
        public List<StbBrace> StbBrace
        {
            get
            {
    ...
    public partial class StbSlabs
    {
        ...
        private List<StbSlab> stbSlabField;

        /// <remarks/>
        [System.Xml.Serialization.XmlElementAttribute("StbSlab")]
        public List<StbSlab> StbSlab
        {
            get
            {
    ...
    public partial class StbSlabOffsetList
    {
        ...
        private List<StbSlabOffset> stbSlabOffsetField;

        /// <remarks/>
        [System.Xml.Serialization.XmlElementAttribute("StbSlabOffset")]
        public List<StbSlabOffset> StbSlabOffset
        {
            get
            {
    ...
    public partial class StbWalls
    {
        ...
        private List<StbWall> stbWallField;

        /// <remarks/>
        [System.Xml.Serialization.XmlElementAttribute("StbWall")]
        public List<StbWall> StbWall
        {
            get
            {
    ...
    public partial class StbWallOffsetList
    {
        ...
        private List<StbWallOffset> stbWallOffsetField;

        /// <remarks/>
        [System.Xml.Serialization.XmlElementAttribute("StbWallOffset")]
        public List<StbWallOffset> StbWallOffset
        {
            get
            {
    ...
    public partial class StbFootings
    {
        ...
        private List<StbFooting> stbFootingField;

        /// <remarks/>
        [System.Xml.Serialization.XmlElementAttribute("StbFooting")]
        public List<StbFooting> StbFooting
        {
            get
            {
    ...
    public partial class StbStripFootings
    {
        ...
        private List<StbStripFooting> stbStripFootingField;

        /// <remarks/>
        [System.Xml.Serialization.XmlElementAttribute("StbStripFooting")]
        public List<StbStripFooting> StbStripFooting
        {
            get
            {
    ...
    public partial class StbPiles
    {
        ...
        private List<StbPile> stbPileField;

        /// <remarks/>
        [System.Xml.Serialization.XmlElementAttribute("StbPile")]
        public List<StbPile> StbPile
        {
            get
            {
    ...
    public partial class StbFoundationColumns
    {
        ...
        private List<StbFoundationColumn> stbFoundationColumnField;

        /// <remarks/>
        [System.Xml.Serialization.XmlElementAttribute("StbFoundationColumn")]
        public List<StbFoundationColumn> StbFoundationColumn
        {
            get
            {
    ...
    public partial class StbParapets
    {
        ...
        private List<StbParapet> stbParapetField;

        /// <remarks/>
        [System.Xml.Serialization.XmlElementAttribute("StbParapet")]
        public List<StbParapet> StbParapet
        {
            get
            {
    ...
    public partial class StbOpens
    {
        ...
        private List<StbOpen> stbOpenField;

        /// <remarks/>
        [System.Xml.Serialization.XmlElementAttribute("StbOpen")]
        public List<StbOpen> StbOpen
        {
            get
            {
    ...
    public partial class StbOpenIdList
    {
        ...
        private List<StbOpenId> stbOpenIdField;

        /// <remarks/>
        [System.Xml.Serialization.XmlElementAttribute("StbOpenId")]
        public List<StbOpenId> StbOpenId
        {
            get
            {
    ...
    public partial class StbExtensions
    {
        ...
        private List<StbExtension> stbExtensionField;

        /// <remarks/>
        [System.Xml.Serialization.XmlElementAttribute("StbExtension")]
        public List<StbExtension> StbExtension
        {
            get
            {
