...
namespace ADSK.JExtRAC.AveSiteLevelHeightCalc.Components
{
    public partial class Geometry
    {
        ...
        public XYZ GetElementLocPos(Element element)
        {
        ...
        public double Distance(XYZ a, XYZ b) => a.DistanceTo(b);

        public double CrossProduct2D(XYZ o, XYZ a, XYZ b)
        {
            return (a.X - o.X) * (b.Y - o.Y) - (a.Y - o.Y) * (b.X - o.X);
        }

        public XYZ CrossProduct(XYZ p0, XYZ p1, XYZ p2)
        {
            XYZ u = p1 - p0;
            XYZ v = p2 - p0;
        ...
        public XYZ TriangleGravity2D(XYZ pos1, XYZ pos2, XYZ pos3)
        {
            if (Distance2D(pos1, pos2) < Approx0Len || Distance2D(pos2, pos3) < Approx0Len || Distance2D(pos3, pos1) < Approx0Len)
                return null;
            var curves = new List<Curve>
            {
                Line.CreateBound(pos1, pos2),
                Line.CreateBound(pos2, pos3),
                Line.CreateBound(pos3, pos1)
            };
            return PolygonGravity2D(curves);
        }

        public SetComparisonResult CompareInterCurve(Curve c1, Curve c2, ref IntersectionResultArray arr)
        {
            arr = null;
            using (CurveIntersectResult cir = c1.Intersect(c2, CurveIntersectResultOption.Detailed))
            {
                return cir.Result;
            }
        }

        public void GetTriMeshTopoSurface(TopographySurface topoSurface, ref IList<MeshTriangle> triMeshes)
        {
            triMeshes ??= new List<MeshTriangle>();
            if (topoSurface == null)
                return;
            var opt = _rvtUIDoc.Application.Application.Create.NewGeometryOptions();
            opt.DetailLevel = ViewDetailLevel.Medium;
            GeometryElement geo = topoSurface.get_Geometry(opt);
            GetTriMeshElem(geo, ref triMeshes);
        }

        public void GetTriMeshTopoSurface(Toposolid toposolid, ref IList<MeshTriangle> triMeshes)
        {
            triMeshes ??= new List<MeshTriangle>();
            if (toposolid == null)
                return;
            var opt = _rvtUIDoc.Application.Application.Create.NewGeometryOptions();
            opt.DetailLevel = ViewDetailLevel.Medium;
            GeometryElement geo = toposolid.get_Geometry(opt);
                ...
                if (geoObject is Solid solid)
                {
                    foreach (Face face in solid.Faces)
                    {
                        ...
                        for (int i = 0; i < mesh.NumTriangles; ++i)
                            triMeshes.Add(mesh.get_Triangle(i));
                    }
                }
            }
        }

        private void GetTriMeshElem(GeometryElement geomElem, ref IList<MeshTriangle> triMeshAry)
        {
            if (geomElem == null || triMeshAry == null)
                return;
            foreach (GeometryObject geoObj in geomElem)
            {
                if (geoObj is Mesh mesh)
                {
                    ...
                    for (int i = 0; i < n; ++i)
                        triMeshAry.Add(mesh.get_Triangle(i));
                }
                else if (geoObj is GeometryInstance gi)
                {
                    GetTriMeshElem(gi.GetInstanceGeometry(gi.Transform), ref triMeshAry);
                }
            }
        }
    }
